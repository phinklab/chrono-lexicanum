...
snapshot-reason: Pre-Karpathy-reset (brief 049). Top-level ARCHITECTURE.md was the system's living architecture description until the reset; canonical engineering memory now lives in brain/wiki/. This file preserves the post-048-doc-refresh state (Phase 3 / TypeScript / Wikipedia + Lexicanum + Open Library + Hardcover + LLM stack) for archaeological reference.
