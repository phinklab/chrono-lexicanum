---
snapshot-of: ROADMAP.md
snapshot-date: 2026-05-08
snapshot-reason: Pre-Karpathy-reset (brief 049). Top-level ROADMAP.md was the canonical phased plan until the reset; canonical engineering memory now lives in brain/wiki/. This file preserves the Phase-3-in-flight state (post-Plan-Reshuffle 2026-05-02) for archaeological reference.
canonical-now: brain/wiki/roadmap.md
---
...
- [x] **Phase 3a — Bulk-Backfill-Skeleton** (sessions 034/035) — Wikipedia-Discovery + Lexicanum-Crawler + Multi-Source-Engine + Field-Priority-Map + Manual-Protection-Comparator + Resumable-State + CLI mit `--limit N`. Dry-Run-only.
- [x] **Phase 3b — Open Library + Hardcover.app + Schema-Erweiterung** (sessions 036/037) — zusätzliche Quellen an die Engine angedockt. Migration 0005 mit `format`/`availability`-Enums + `isbn10` + `pageCount` auf `book_details`. Wikipedia-Discovery erweitert auf 4 Pages (701 unique Bücher).
- [x] **Phase 3c — LLM-Anreicherungs-Schicht** (sessions 038/039 + Brief 040) — Anthropic LLM mit Web-Search; Synopsen-Paraphrase, Soft-Facets, Format/Availability-Klassifikation, Plausibility-Cross-Check, Reader-Rating-Capture. Migration 0006 (rating-Felder). Test-Gate-Entscheidung 2026-05-04 zugunsten **Haiku 4.5** (3.2× günstiger als Sonnet, $88 vs $280 für 800 Bücher); Brief 040 schließt 3c sauber ab mit Modell-Switch + Vokabular-/Format-Prompt-Härtungen + Re-Test auf Bücher 21–40.
- [ ] **Phase 3.5 — Ingestion-Dashboard** (Brief 041) — Read-only Route `/ingest`, liest committed Diff-Files aus `ingest/.last-run/*.diff.json`, zeigt Summary-Cards pro Lauf + per-Buch Drill-down. Filesystem-basiert, kein DB-Persist. Voraussetzung für die batched 3e-Strategie.
- [ ] **Phase 3d — Apply-Step** — Diff-File → DB-Writes mit `ON CONFLICT … WHERE source_kind != 'manual'`. Bringt UNIQUE INDEX `external_links`, `junctionsLocked: true`-Flag, FK-Resolution für `work_persons`/`work_facets`/`external_links`-Junctions, ALTER TYPE `source_kind` ADD VALUE für `open_library`/`hardcover`/`llm`. Liest zusätzlich Override-File aus Philipps externer Hand-Check-Arbeit (siehe nächster Folge-Brief 042).
- [ ] **Phase 3e — Batched Backfill (~800 Bücher)** — Strategie-Anpassung 2026-05-04: statt einem 800-Buch-Übernachtlauf jetzt **8–16 Sessions à 50–100 Bücher** über mehrere Wochen, manuell getriggered via `--limit N --offset M`. Nach jeder Session: Diff-Inspektion auf dem Ingestion-Dashboard (3.5), gezielter Hand-Check via Claude.ai für Prestige-Reihen + Bücher mit auffälligen Flags, Cowork-Triage der LLM-Flags. Bessere Quality-Gate als Übernachtlauf, schonender für den PC.
