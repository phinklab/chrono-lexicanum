...
## 2026-07-01 · Coordination · Rollup 170/171/172 — Per-Buch-SSOT-Migration + Podcast-Delta

**Quelle:** `sessions/2026-06-28-170-impl-per-book-ssot.md` + `2026-06-30-171-impl-per-book-ssot-migration.md` + `2026-07-01-172-impl-podcast-weekly-maintenance.md` (alle drei gemergt: PR #201 = 170, 171 gemergt 2026-07-01, PR #204 = 172; alle `implemented`). Adressiert den offenen Punkt aus dem `sessions/README`-Nachtrag 2026-07-01 („fuller Koordinations-Pass-Ingest steht noch aus"). project-state stand vorher auf 2026-06-26 / „889 Bücher" und beschrieb die Batch-Maschinerie noch als lebend — das war der Kern-Fix.

**Working-Tree-verifiziert (nicht nur den Reports geglaubt):**
- **896 Bücher** in `scripts/seed-data/books/*.json` (599 `W40K-*` bis `W40K-0599` + 297 `HH-*` bis `HH-0297`), **59** mit non-empty `collections.collects[]` = **196 Collection-Kanten** (per-`json`-Parse gezählt).
- `db-sync.sh` ist **9-schrittig**: Schritt 1 `book-corpus-preflight`, Schritt 2 `apply:book --all --mode post-retirement` (primärer Korpus-Schritt), dann Tails podcast/audiobook(+verify)/timeline(+verify)/curation-overlay(+verify). Kein `run-phase4-apply.sh`-Korpus-Schritt mehr.
- Retirement-Stubs verweigern laut + zeigen auf den Per-Buch-Pfad: `apply-override.ts`→`apply:book`/`equiv:diff`, `db-apply-scope.ts`→`book:preflight`, `loop-next-batch.ts`, `run-ssot-loop.sh`, `import-ssot-roster.ts`.
- Neue `package.json`-Scripts vorhanden: `apply:book`, `book:preflight`, `migrate:corpus-to-books`, `equiv:diff`, `books:excel`. `.claude/commands/{add-podcast-episode,add-podcast,weekly-db-update}.md` + `src/lib/ingestion/podcast/delta.ts` vorhanden. Frozen Provenienz on-disk: `book-roster.json`, `book-roster.extension.json`, 90 `manual-overrides-ssot-*`, `source/Warhammer_Books_SSOT.xlsx`.

**Eingefaltet:**
- **170 Teil A (PR #201):** additives per-Buch-Fundament — `book-v1`-Format, geteilter Writer `book-apply-shared.ts` (verbatim aus `apply-override.ts` extrahiert), targeted `apply:book --slug/--all/--verify`, geteilter Prolog `seed-prolog.ts`, `effective-corpus.ts` für Refresh-Awareness, `/add-book`-Runbook. Strikt additiv, `books/` schiffte leer.
- **171 Teil B:** Einmal-Konverter `migrate:corpus-to-books` (896 Files geschrieben, 59 mit `collects`), Äquivalenz-Gate `equiv:diff` — **leerer DB-freier Projektions-Diff** (896 Bücher / **0 Row-Deltas** / 196/196 Collection-Kanten), Parität-by-construction am geteilten `computeBookRows`. Serien-Parität exakt auf die 8 `SERIES_BY_EXTERNAL_ID`-Anker (nie aus `seriesHint`). **Erst dann** Retirement der Batch-/Excel-/Loop-/Roster-Extension-CLIs (Daten eingefroren, nicht gelöscht), `apply:book --all` → primärer Korpus-Schritt, neuer `book-corpus-preflight` ersetzt die `db:apply-scope`-Contiguity-Claim, read-only `books:excel`-Export. Drei `CorpusMode`s (`additive`/`migration-equivalence`/`post-retirement`). Kein Prod-Write (Gate ist DB-frei; `--db-snapshot`/`--compare` nur gegen disposable DB, prod-refusal-guarded).
- **172 Teil C (PR #204):** additiver Podcast-**Delta**-Pfad (`delta.ts` + `prepare-delta`/`merge-delta`-Subcommands, nur neue GUIDs, No-Shrink-/No-Retag-Guards, `DeltaGuardError`→exit 2), targeted Apply bleibt `apply:podcast --show`; Wartungs-Commands `/add-podcast-episode`, `/add-podcast`, `/weekly-db-update` als Runbooks + `.claude/commands/*.md`; Weekly-Refresh-Promote-Texte auf den Delta-Pfad reconciled (stale `db:apply-override` → `apply:book`/`apply:podcast` korrigiert). CI bleibt detection-only, `db:drift` read-only, kein `ANTHROPIC_API_KEY`-Pfad, keine retirte Maschinerie reaktiviert.

**OQ-Bewegung:** **18a** (via 170 Teil A, PR #201) + **18b** (via 171 Teil B) beide geschlossen — waren in `open-questions.md` bereits im Kopf/Schluss-Vermerk als geschlossen geführt; dieser Pass bumpt nur noch die Frontmatter-`updated` auf 2026-07-01. Queue steht jetzt auf **(16b/c)** allein. Keine neue OQ aus 170/171/172.
...
## 2026-07-02 · Coordination + Brief 183 · Impl-174 gelesen, Map-Daten-Pass vor dem UI-Schnitt

**Gelesen:** Impl-Report `2026-07-02-174-impl-map-ssot-reconciliation.md` (complete, PR #209 gemergt). Kernbefund: 87/992 Excel-Welten matchen (80 Locations, 46 % der 1685 Werk-Kanten); Nachplatzierungs-Worklist 363 Einträge; Ableitung driftfrei (0 Extra-Kanten, DB hängt Korpus um 6 nie applizierte Bücher hinterher — löst sich beim nächsten `apply:book --all`).

**Cowork-Analyse:** Fuzzy-Pass (offene Frage aus Impl 174) ist **tot** — saubere Normalisierung bringt 2/363 Treffer (Dal'yth, The Black Library); die Excel enthält die fehlenden Orte schlicht nicht (aktuelle-40k-Galaxis: keine Heresy-/zerstörten Welten, Regionen, Unterorte, Nicht-Orte). Worklist ist langschwänzig: 212/363 mit genau 1 Werk; Top-Kategorien: Unterorte gematchter Welten, Namens-Varianten existierender Excel-Welten, echte fehlende Planeten, Regionen, Nicht-Orte.

**Philipp-Entscheide (zwei Runden):** Rollup-Mechanik bauen (inkl. Regionen → Repräsentativ-Welt); Daten-Pass läuft **vor** 178 (P14 damit Dreiteiler 174/183/178); Cowork recherchiert Koordinaten-Vorschläge, Philipp reviewt im PR. Rev. 2. Runde: **Excel = SSOT, nichts limitieren** — alle 992 Elemente (Flotten, Webway-Tore, Warp-Stürme, Blackstones) bleiben anzeigbar/filterbar, KEINE Ausschluss-Liste; Hand-Kuration in einer committeten **Kurations-Excel** als Convert-Input (ersetzt `map-worlds.overrides.json`; Philipps Arbeitsformat, nachvollziehbar); kuratierte **`kind`-Typ-Gruppen** als Daten-Kontrakt für Icons/Farben/Filter. Cowork-Spaltenbefund: Excel hat KEINE Beschreibungs-Spalte (Beschreibungen weiterhin nur aus unserem Bestand via `locationId`); Primary Classification = 70 Werte, Secondary auf 172 Zeilen.

**Neuer Brief:** `sessions/2026-07-02-183-arch-map-worlds-daten-pass.md` (open, Batches, DB-frei) — Kurations-Excel `source/map-worlds-curation.xlsx` (Sheet „Kuration" media-keyed mit Aktion link|rollup|pin|später, alle 363 Zeilen nach Werkzahl sortiert, 44 vorbefüllt; Sheet „Welten" für Dubletten-Entkopplung; fail-loud), Rollup-Mechanik (`via`-Herkunft, Dedup, stärkere Rolle gewinnt), `kind`-Feld (70 Classifications → 11 Gruppen + `region`, verbatim-Mapping im Brief, unbekannter Wert = fail-loud) + `classification2`, verbatim Kurations-Daten: 4 Namens-Links (T'au, Dalyth, Beta Garmon IV, Black Library), 20 Rollups (u. a. Imperial Palace/Sol/Luna→Terra, Hives→Necromunda, Ultramar→Macragge, Ullanor→Armageddon, Sortiarius→Prospero, Vengeful Spirit→Eye of Terror), 20 Hand-Pins mit Lexicanum/Fandom-recherchierten Koordinaten im 174er-Grid (Istvaan III/V, Sotha, Iax, Nostramo, Tsagualsa, Cryptus, Alecto, Ulthwé, Sabbat-Cluster, Scarus-Cluster, Damocles, Gothic, Ghoul Stars). Keine Ausschlüsse — Webway/Great Rift/Imperium Nihilus/Eastern Fringe bleiben bewusst offen in der Worklist. Erwartete Abdeckung ~1200/1685 Kanten (~71 %). Rest-Tail ~315 Einträge = laufende Hand-Kuration. OQ 16b/c explizit deferred (unverwandt).

**Edits (Cowork, uncommitted — reiten im nächsten CC-PR):** Brief 183 (neu), `sessions/README.md` (Kopf Forward-Queue, Zeile 183, 174-Zeile), `worklist.md` (§ A Dreiteiler + § F Worklist-Merker), `project-state.md` (§ What's open + § Next likely brief P14), `log.md` (dieser Eintrag).

## 2026-07-02 · Coordination · Impl 179 (Launch-Legal) gelesen + gebucht

**Gelesen:** Impl-Report `2026-07-02-179-impl-launch-legal.md` (complete, gemergt, Philipp-bestätigt). Impressum + Datenschutzerklärung + GW-Disclaimer live, ungated; Footer-Legal-Row, `/login`-Zeile, Burger-Menü-Links, plus Philipp-Nachtrag `SiteLegal`-Zeile unter dem Player (vorläufig, footerlose Desktop-Flächen).

**Substanzielle CC-Entscheide validiert:** Slugs final englisch `/imprint`/`/privacy` mit deutschen Link-Labels (P12-Ausnahme obsolet — Worklist-Hinweis korrigiert); GW-Disclaimer als Impressum-Sektion `#disclaimer` statt eigener Seite; § 36 VSBG aufgenommen, EU-ODR-Link bewusst weggelassen (Plattform 20.07.2025 eingestellt); kein Consent-Banner (einziger Cookie § 25 Abs. 2 Nr. 2 TDDDG, Analytics cookielos formuliert — 182 braucht keinen Legal-Nachzug); keine Platzhalter (Angaben standen im Brief).

**Buchhaltung:** README-Zeile 179 → implemented (+ Kopf-Queue); `worklist.md` § A-3 (179 gestrichen) + § A-5 (P12-Hinweis korrigiert); `deferred-questions.md` § Preview-Gate: zugesagte Löschung der `preview_invite_activations`-Zeilen + noindex-Fall beim Flip ergänzt (aus „For next session"); `docs/ui-backlog.md`: SiteLegal-Polish-Slot + 178-Kollisionscheck aufgenommen. `open-questions.md` unverändert (keine neue OQ).

## 2026-07-03 · Coordination + Brief 184 · Impl-183 gelesen, De-Slop-Design-Reset vor dem Map-UI

**Gelesen:** Impl-Report `2026-07-02-183-impl-map-worlds-daten-pass.md` (complete, gemergt, inkl. 2 Nachträge). Kurations-Excel ersetzt `map-worlds.overrides.json`, Katalog `map-worlds-v2` (kind 11+`region`, `via`, `coverage`), 44/44 Tests. Nachtrag 1: Pin-Koordinaten auf SSOT-Pixelraum umgestellt (copy-paste-kompatibel zur Redditor-Excel, Projektion eingefroren, Katalog byte-identisch). Nachtrag 2: Maintainer-Delegation der Rest-Kuration an CC — +1 Link/+38 Rollups/+42 Pins → **1054 Welten, Abdeckung 1332/1685 = 79,1 %, Worklist-Rest 238** (Kanon-Grenze, nicht Fleiß-Grenze). Substanzielle CC-Entscheide validiert: `eye-of-terror`-Slug-Auflösung, flaches `classification`-Feld als 178-Kontrakt, strenger Link-Guard, Rollup-Dedup „eigener Eintrag gewinnt bei Gleichstand".

**Philipp-Entscheid (2026-07-03, revidiert Worklist-§-E-Timing vom 01.07. teilweise):** Die Site liest trotz gelungener Elemente als „AI-Slop" (Fonts, Typo-Anordnung, Rauten-/Icon-Dekor, Button-Darstellung) — der **Design-Language-Reset läuft vor 178**, damit die Map einmal in der neuen Sprache gebaut wird statt re-skinned. Methode (Cowork-Empfehlung, Philipp-bestätigt): **lokale Standalone-HTML-Richtungsstudien via CC/Fable** statt Claude Design als Hauptpfad (echte Inhalte/Token-Nähe, derselbe Agent entwirft + implementiert, direkt editierbar; Claude Design optional als Feeder). Flächen (Philipp-Auswahl): Archiv-Liste, Buch-Detail, Map-Mock — bewusst ohne Home. Nur der Language-Reset ist vorgezogen; UI-Gesamt-Pass-Rest (A11y, Farbsprachen, P7-Rest) bleibt geparkt.

**Neuer Brief:** `sessions/2026-07-03-184-arch-design-richtungsstudien.md` (open, Product) — 2–3 divergente Richtungen × 3 Flächen als Doppelklick-öffnenbare Standalone-HTML in neuem Top-Level-Ordner `design/` (nicht `public/`, keine `/lab`-Routen; Präzedenz `timeline-workshop/`), echte Daten aus `books/*.json` + `map-worlds.json`, je Richtung ein Rationale-Block, Zero-Diff außerhalb `design/**`. Design-Freedom maximal: Ist-Optik ausdrücklich nicht bindend. Kette: 184 → Richtungswahl → **Brief 185 Design-Language-Pass** (Tokens/Base, Cowork schneidet danach) → 178 → 181. OQ 16b/c explizit deferred (unverwandt).

**Edits (Cowork, uncommitted — reiten im nächsten CC-PR):** Brief 184 (neu), `sessions/README.md` (Nachtrag 2026-07-03, Zeile 183 → implemented, neue Zeile 184), `worklist.md` (§ A: 183 gemergt + De-Slop-Zweischritt als Punkt 2, Renummerierung 3–8; § E Teil-Promotion; § F Worklist-Merker 238), `project-state.md` (§ What's open P14, § Next likely brief), `log.md` (dieser Eintrag).

---

## 2026-07-04 · Coordination · Status-Reconcile + 177-Dead-Code-Rollup (Merge-Stand 175–184 + Design-Reset live)

Von Philipp beauftragter „wo stehen wir"-Pass gegen den echten Git-Stand (`git log --oneline` + `gh pr list`, von Philipp geliefert — Cowork fasst kein git an). Befund gegen das Brain: fünf Briefe waren `open`, real aber alle **gemergt** (2026-07-03): **175** Podcast-Hygiene (PR #211), **176** Roster-Rebind + B7 (PR #212), **177** Dead-Code-Sweep (PR #214, sein `brain:lint`-Blocker separat via PR #213), **180** CI-Test-Gate + Wartung (PR #215), **184** Design-Richtungsstudien (PR #222). Plus Dependabot-Erstbumps #216–#219.

**Wendepunkt (184):** Der Design-Reset blieb nicht bei Studien — nach **10 Review-Runden** hat Philipp Richtung **„Kathedrale"** gewählt und CC hat sie in **Runde 10 direkt in `src/**` live gebracht** (Home/Archive/Compendium/Ask + geteiltes Fundament: `next/font` Cormorant SC/Cardo/Fragment Mono, Palette `#050301`, SiteNav-Rail + Rubriken-Strich, Vox-Schreiber, `MainAuspex`/`AuspexPair` ohne Sweep, Sternwarte-Button-System `BtnFx`, „Kapelle"-Popup über den `@modal`-Intercept; viel Alt-Chrome gelöscht — HeroScrollCue/Typewriter/LiveTelemetry/BottomConsole/FactionClassIcon …). Chronicle erbt nur Tokens/Fonts, Entity-Seiten + Map strukturell unberührt. **Konsequenz:** der geplante **Brief 185 (Design-Language-Pass) ist hinfällig** — sein Zweck ist in 184 erfüllt; offen bleibt nur Rest-Restyle (Entity/Chronicle/Mobile) nach der Map. **Nächster Cowork-Schnitt: Brief 178 Map-Neubau** in der neuen Sprache (Design-Reset-Blocker weg).

**177-Dead-Code-Rollup (Cowork-Nachlauf, dieser Pass):** Der Sweep hat V1-Crawler + V2-LLM-Engine physisch gelöscht (43 Dateien, −8.994 LOC); `brain/wiki`-Prosa, die diese Module noch als lebend beschrieb, auf Historie gezogen: `pipeline-state.md` (Retirement-Banner am Kopf), `glossary.md` (Pipeline-Engine / Manual-Protection-Comparator / SourceCrawler / batched-3e als retired-177 markiert), `architecture.md` + `onboarding.md` (ingest:backfill-Verweise), ADR `why-cc-direct-curation.md` § „V2-LLM-Stage bleibt im Repo" (Option-A-Reaktivierungs-Sicherung ist physisch entfallen — Reaktivierung hieße Rebuild aus git-Historie) + `why-sonnet-not-haiku.md`. `brain:lint` war bereits grün (dangling `sources:` via PR #213 entfernt).

**176-Nachlauf:** `workflows/lint.md` um die Kategorie „Always-read budget" ergänzt (B7 shipped). `sessions/README.md` unter das 14k-Soft-Budget geprunt (fünf gestapelte Nachträge → ein konsolidierter 2026-07-04-Kopf).
...
## 2026-07-15 · Coordination · Werkstatt-Phase verankert + Reconcile bis Session 218 (Session 219)

**Basis:** Branch `codex/session-219-werkstatt-verankerung` von `origin/main` (`55a8ea7`, PR #264 gemerged). Report: `sessions/2026-07-15-219-impl-werkstatt-verankerung.md`.

**Plan-Nachtrag (Philipp, 2026-07-14/15), in `docs/launch-master-plan.md` § Entscheidungen als W1–W6 festgeschrieben:** kein Launch-Zeitdruck — Release erst bei vollständigem Artist-Artwork **und** vollständig *besuchter* Feature-Wunschliste (Urteil je Idee: bauen / Backlog / verwerfen); Werkstatt-Phase vor dem Gate mit Einstieg Idee 2 → 1 → 3c, dann 4, 3a, 3b, 5, 6 + Anhangs- und Perfektions-Triage (Schema-Änderungen zulässig, Ideas-Backlog-first bleibt); S7b + S11-Code-PR laufen VOR dem Launch (finale S7b-Live-Messung nach Gate-off); Preview-Gate + Invite-Maschinerie tragend bis Launch, PL1 nie vorher; Release-Endspiel fix (Freeze → Readiness → Flag-Flip → stilles Fenster → Reddit-Post); E8 verlängert bis einschließlich Launch-Readiness. Ideenliste vom 2026-07-13 als `docs/post-launch-feature-ideas.md` committed; Schema-Ideen 3a/5/6 im neuen Ideas-Backlog der `roadmap.md`.

**Reconcile auf den echten main-Stand:** Muss-Strecke S0 + S1a–S10a gemerged (Sessions 194–209, PRs #240–#256). Map-Welle 210–218: statische Dash-Route + Compositing-Fix (#257–#259); Android-Canvas-Renderer geräteabgenommen (213/#260) — schließt das 192-Verdict, **S10b faktisch erledigt**; Curator-Rename + Fraktionspfad + Legal EN/`?lang=de` (214/216, #262/#263); Great Journeys → 11 Reisen / 231 Acts mit 64 quellenzitierten Chart-Points + 13 Black-Crusade-Sorties (214/215/217/218, #261/#264). Worklist-Auflösungen: Cameo/`ask-fraktion` abgedeckt (ofob-Prototypen in S5/#251 entfernt, Live-Hierarchie aus 214/216); Legal-i18n durch 214 gelöst (kein next-intl); signierte Audio-URLs durch S3b erledigt; Migrations-0015-Check bleibt als Readiness-Punkt-1-Verweis. Worklist-Reste als „Perfektions-Kandidaten" der Werkstatt-Triage gekennzeichnet.
