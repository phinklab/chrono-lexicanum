...
- **DetailPanel "Auch enthalten in:"-Mini-Brief.** Backend nach 072-Cross-Batch-Refactor vollständig (35 `work_collections`-Rows, davon einige content-seitig cross-batch); Brief 074 prüft nur den globalen Status und hält Green Tide als Collection-Gap-Ledger-Fall fest, ohne partielle Kanten zu schreiben. Frontend-Query ist UI-Session — wahrscheinlich Teil eines Cockpit-Detail-View-Refinements.
...
|---|---|---|---|
| 2026-05-15 | 074-arch | open | Resolver-Pass 3 — Surface-Form-Crystallization für `ssot-w40k-011..015`. Brief geschrieben, eigener Branch, Watson-Trilogy historical-canon-layer-Entscheidung dokumentiert, Green Tide bleibt Buch-Scope und wird als `collection-gaps.json`-Ledger-Fall statt partieller `work_collections`-Kanten festgehalten. CC-Job offen. |
