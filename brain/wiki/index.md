...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- | ---------- |
| [project-state.md](./project-state.md)           | "Where are we now": phase, what's running, next likely brief. Post-Merge-Koordinations-Pass post-PR-109 (2026-05-28): Brief 103 Daten-Pass gemergt — `npm run audit:gap-candidates`-Helper (read-only Triage, 325 Kandidaten Raw-Output HH=73 W40K=252) + zwei Pilot-Backfills HH-0260 *Hunter's Moon* (Russ + Alpharius als `mentioned`, neuer `pelago`-Location-Anker) und HH-0270 *Iron Corpses* (neuer `koparnos`-Character-Anker); NEW-Range gap_works 18→16. Reference characters 489→490, locations 288→289. Brief 103 selbst `status: implemented` (CC hat geflippt im PR-Commit, idempotent-Konvention — UI-Pass bleibt offen). **Korpus bleibt datenkomplett + konsolidiert (859/859).** Nächster Architekten-Brief: drei Kandidaten — (a) Brief 103 UI-Pass an CC übergeben (Top-Empfehlung, kein neuer Brief), (b) Phase-3-Seal-Brief, (c) Aufmerksamkeit zurück zu Brief 096. | 2026-05-28 |
| [open-questions.md](./open-questions.md)         | Items the next architect brief must address. Post-Merge-Koordinations-Pass post-PR-109 (2026-05-28): Brief 103 Daten-Pass gemergt — keine OQ-Schließung (Daten-Pass + Tooling-Add ohne numerierten OQ-Bezug; alle sechs Brief-103-§-Open-questions im Daten-impl beantwortet — Q1+Q4 dort, Q2/Q3/Q5/Q6 sind UI-Pass-Themen), keine neue numerierte OQ. Offene Queue unverändert: OQ (3) Hand-Check-Workflow, OQ (13) Crawl-Simplification-Sichtung. Watch-Items im § What's open in `project-state.md`. | 2026-05-28 |
...
| [pipeline-state.md](./pipeline-state.md)         | Phase-3 pipeline detail: V1 legacy + V2 SSOT authority path + resolver layer. Post-PR-109 (2026-05-28): Brief 103 Daten-Pass gemergt — neues read-only Triage-Tool `npm run audit:gap-candidates` über `works ⨝ book_details` (325 Kandidaten Raw-Output, HH=73 W40K=252) + zwei Pilot-Override-Backfills HH-0260 / HH-0270 senken NEW-Range gap_works 18→16. Reference locations 288→289 (+pelago), characters 489→490 (+koparnos). Brief-103-Forecast-Korrektur: "~10–20 echte fixable" galt für post-Triage-Subset, nicht den Helper-Raw-Output. UI-Pass bleibt offen (Product-Worktree). | 2026-05-28 |
...
| [log.md](./log.md) | Append-only operation log (chronological history of Brain edits) | 2026-05-28 |
| (this file) | Master catalog | 2026-05-28 |
