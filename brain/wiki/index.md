...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- | ---------- |
| [project-state.md](./project-state.md)           | "Where are we now": phase, what's running, what's open, next likely brief. **Stand 2026-06-24:** Korpus 889, 4 Podcast-Shows (~1094 Episoden); **Buch-Reviewer (B11) voll gelaufen + appliziert** (154 + Stage 3 155 + Gate F/L 158: +20 Factions / +142 Locations in der Prod-DB; 53 sektor-zugeordnete Welten = Map-Worklist); **Product-Wave 159–163** (Universal Search, neuer BG, Perceived-Latency, Entity-ISR, Timed-Preview-Invite-Links). DB-Apply-Modell `db:sync`/`db:drift`/`db:rebuild`. Ops sauber (db:sync + Migration 0014 + beide Secrets bestätigt). Site hinter Preview-Gate (+ Invite-Links). Forward: P14 Map → B12 Ask → P12 URL-EN → P13 Mobile. | 2026-06-24 |
| [open-questions.md](./open-questions.md)         | Items the next architect brief must address (Queue klein halten, 3-5). Aktuell: **(16b/c)** Timeline-Folgen aus impl 137/152: `primaryEraId`-Placeholder + Atlas-Events; **(18a/b)** vertagte Apply-Vertiefungen aus Brief 157: `db:apply` Plan/Diff + exakter DB==SSOT-Deep-Diff. Wave 154–163 öffnete keine neue OQ. **16(a) seit Brief 152 geschlossen**; (17) 2026-06-12 geschlossen -> Boards. | 2026-06-24 |
...
| [log.md](./log.md) | Append-only operation log (chronological history of Brain edits) | 2026-06-24 |
