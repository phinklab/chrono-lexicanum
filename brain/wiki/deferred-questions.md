...
## Geparkte Dependency-Major-Bumps — ESLint 10 & `@types/node` (Dependabot)

**Owner:** none (Wartungs-Trigger, kein Brief). **Sessions:** [180-impl](../../sessions/2026-07-03-180-impl-ci-test-gate-wartung.md) (Deps-Rundgang; ESLint-10-Block empirisch bestätigt), Dependabot-Erstlauf 2026-07-03 (#219–#221), Maintainer-Diskussion 2026-07-03 (zod-4-Review CC-verifiziert grün).

Zwei Dependabot-Major-PRs bleiben bewusst offen/geparkt. **Niemals `@dependabot ignore this major version` kommentieren** — das killt die künftige Erinnerung an genau diesen Major. Plain-Close (ohne `ignore`) parkt sie; Dependabot öffnet sie beim nächsten Release automatisch neu → das ist der wiederkehrende Reminder, gratis. **Kein eigenes Checker-Skript nötig:** der CI-Job `lint-and-typecheck` läuft auf jedem PR (`npm run lint` / `typecheck` / `npm test`, Node 22) — die CI-Ampel des PRs *ist* die Merge-Readiness.

- **ESLint 9 → 10 (#220).** Upstream-blockiert: `eslint-config-next@16.2.x` bündelt `eslint-plugin-react` / `-import` / `-jsx-a11y`, deren neueste Releases nur `eslint ^9` peeren (kein `^10` existiert). ESLint 10 entfernt `context.getFilename()` → `eslint-plugin-react 7.37.5` crasht zur Laufzeit, `npm run lint` wird **rot** (in Impl 180 empirisch verifiziert). `overrides` kann das nicht fixen (kompatible Plugin-Versionen sind schlicht nicht veröffentlicht). ESLint-9-EOL (2026-08-06) heißt nur „keine Core-Patches", nicht „hört auf zu funktionieren". **Signal:** die CI-Ampel des ESLint-10-PRs springt grün (bzw. ein `eslint-config-next`-Bump-PR taucht auf, dessen gebündelte Plugins `^10` peeren). Dann ist der Bump ein Ein-Zeilen-devDep-Change + `npm run lint`-Check.
- **`@types/node` 22 → 26 (#221).** **Kein Security-Item** — types-only devDep, shippt nichts, führt nichts aus. Das Runtime ist Node 22 (`.nvmrc` + `engines >=22`), ein aktuell supporteter Maintenance-LTS (EOL 2027-04-30). Der `@types/node`-Major soll den **real laufenden Node-Major spiegeln** (sonst tippt TS Node-26-APIs, die auf Node 22 zur Laufzeit fehlen → grüner Build, Laufzeit-Crash). Bis dahin auf latest **22.x** halten (kommt im grouped `npm-minor-patch`-PR mit). **Promote nur zusammen mit einem bewussten Runtime-Upgrade** — Ziel dann **Node 24 (aktueller Active LTS)**, *nicht* 26 (noch „Current", LTS erst ab 2026-10); dabei `.nvmrc` + `engines` + Vercel-Node-Setting gemeinsam ziehen, dann `@types/node` nachziehen.

**Promote when:** ESLint — CI des (neu geöffneten) ESLint-10-PRs ist grün. `@types/node` — ein dedizierter „Node 22 → 24"-Runtime-Brief wird geschnitten. **Laufende Sichtung** reitet auf dem bestehenden Weekly-Refresh-Pass (`/weekly-db-update`): offene Dependabot-PRs + CI-Ampel prüfen — grün + reviewed → merge, rot → weiter parken (plain-close, kommt wieder).
