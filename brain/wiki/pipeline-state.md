...
# Pipeline state (Phase 3, post-091)
...
### Resolver layer (post-063 through Pass 6 / Brief 091)

The first 350 W40K authority books (`ssot-w40k-001..035`) now have a concrete resolver/apply layer, geschlossen über sechs Resolver-Wellen (063–069 für 001..005, 072 für 006..010, 074-impl für 011..015, 076-impl für 016..020, 089-impl für 021..025, Pass-6-impl für 026..035 — letzte drei **axis-sliced**), plus Brief 077 (Grand-Alignment-Junction-Skip, Faction-Achse) und Brief 084 (Umbrella-Skip, Locations-Achse) als Pipeline-Compliance-Hygiene:
...
- `scripts/seed-data/faction-aliases.json`, `location-aliases.json`, `character-aliases.json`, `characters.json`, plus extended `factions.json`, `locations.json`, `sectors.json` are the checked-in resolver sidecars. Counts post-089: `factions=154`, `locations=169`, `characters=199` reference rows; Faction-Aliases 37, Location-Aliases 13, Character-Aliases 28.
...
21. ✅ **Resolver-Pass 6 für `ssot-w40k-026..035`** (PR #83, `72c8788`) — erster brief-freier, runbook-getriebener Pass (Brief-090-Maschinerie); supervised/axis-sliced gefahren, nicht über den Driver. 350/350 W40K-Bücher applied, Dry-Prediction == DB-POST-APPLY exakt, Trias grün, kein `## Needs decision`. Erste Welle mit forward-referencing collection edges → `apply-override-dry`-Guard auf report-only entschärft (10/10 Edges verifiziert). **Folge: Brief 091** (Resolver-Pass Phase-4-Split 4a/4b + range-aware forward-ref-Guard, `status: open`) — reine Maschinerie-Hygiene, adressiert das Phase-4-Token-Budget (~300k in Pass 6).
...
25. **3e Voll-Apply** (alle 859 Bücher in DB) sobald die Loop-Iterationen genug Confidence haben, mit Resolver-Pflege je 50er-Schwelle.
26. **Refresh-Layer für volatile Felder.** Rating-Refresh + Availability + Cover-URL — separater Pfad mit eigener Cadence; der Goodreads-Page-Read aus Brief 087 ist die etablierte Mechanik (Roadmap-Ideas-Backlog „Refresh-Button").
27. **3f Maintenance-Crawler.** GH-Action monthly Wikipedia/TLBranson-Diff für new releases — *post-SSOT* nur noch als Vorschlags-Quelle für die Excel-Workflow-Iteration.
