...
sources:
  ...
  - ../../scripts/runbooks/resolver-pass-runbook.md
  - ../../scripts/runbooks/ssot-loop-runbook.md
  ...
  - ../../sessions/archive/2026-05/2026-05-27-103-arch-audit-drift-gap-sweep.md
  - ../../sessions/archive/2026-05/2026-05-28-103-impl-data-audit-drift-gap-sweep.md
...
# Pipeline state (Phase 3, post-PR-109 / Brief 103 Daten-Pass — Audit-Helper + 2 Pilot-Backfills, Korpus datenkomplett + konsolidiert 859/859)
...
>
> **⚠ 2026-07-03 — V1-Crawler + V2-LLM-Engine physisch ausgebaut (Brief 177, PR #214). Diese Seite beschreibt ab hier ein RETIRETES Subsystem.** Der Crawler-Stack (`src/lib/ingestion/{wikipedia,lexicanum,open_library,tlbranson,discovery}`, `hardcover/parse`, die V1-Merge-Engine `merge/field-priority/source-confidence/state/dry-run`, die komplette V2-Engine `v2/{run-*,llm,sources,validators,ssot}`, plus `scripts/ingest-backfill.ts` / `synthesize-v2-batch-diff.ts` / `analyze-v2-surfaces.ts`) ist **gelöscht** (43 Dateien, −8.994 LOC). **Lebender Ingest heute:** Per-Buch-SSOT (`scripts/seed-data/books/*.json` → `apply:book`) + Podcast-Delta (`src/lib/ingestion/podcast/**`) + Weekly-Refresh — siehe [`./project-state.md`](./project-state.md). **Was bleibt:** `podcast/**` (live), `types.ts`/`diff-reader.ts` (`/ingest`-Dashboard rendert eingefrorene `ingest/.last-run/*.diff.json`), `hardcover/fetch.ts` + `v2/sources/hardcover.ts` + `v2/types.ts` (Rating-Backfill), der eingefrorene `equiv:diff`-Golden-Pfad + Refusal-Stubs. Alles unterhalb dieser Zeile (Stages, Source-Pairs, Voll-Lauf-Numbers, V2-Schwächen) ist **historisches Archiv der 044–103-Pipeline-Phase**, nicht der aktuelle Code.
>
> **2026-05-30 — Audiobook-Credits durable appliziert (PR #112 / Brief 105 Daten-Pass).** PR #112 (branch `codex/ingest-batches-audiobook-credits`, commit `c9b7c55`) ist gemergt: 66 Bücher (33 HH / 33 W40K) Audiobook-Credits aus dem committed Sidecar `scripts/seed-data/audiobook-narrators.json` (keyed by `externalBookId`, je Credit name/role/sourceUrl/checkedAt/confidence + optional note) via `scripts/apply-audiobook-narrators.ts` (idempotenter, voll-validierender Apply; per-Buch scoped Delete-then-Insert in einer Transaktion; Auflösung über `works.external_book_id` UNIQUE; `--dry-run`/`--file`) auf `persons` + `work_persons` geschrieben. **Junction-Delta:** `work_persons` **785 → 873** (+88 Audio-Rollen: narrator 63 / co_narrator 12 / full_cast 13 über 66 Bücher; 58 single / 5 multi / 3 full-cast); `persons` +29 neue Performer. 4 `no_audiobook`-Audit-Einträge (pre-2008-Titel) nicht appliziert. **P1-Durability-Fix:** der Override-Apply-Delete (`scripts/apply-override.ts` ~L990) ist auf `inArray(role, ["author","editor"])` gescopt — eine Resolver/SSOT-Re-Apply einer audio-bearing Batch lässt narrator/co_narrator/full_cast unangetastet (live verifiziert, `{author:9,narrator:10,co_narrator:4}` identisch vor/nach). Slug-Helper `slugifyPerson`/`deriveNameSort` nach `src/lib/seed/persons.ts` extrahiert (Brief-077-`alignment.ts`-Pattern, Parität zwischen Author- + Audio-Pfad). **Keine Schema-Migration, keine `external_links`-Rows** (Storefront-Links sind UI-derived → Brief-105-UI-Pass). `scripts/test-audiobook-narrators.ts` 12/12 DB-frei; `test:resolver` 473/473 grün (Slug-Extraktion verhaltensgleich). **Cowork-Review: akkurat** (Coverage-Zahlen jq-verifiziert + 3 Credits web-gegengeprüft), **stabil** (idempotent/transaktional, einziger `work_persons`-Delete in apply-override), **sinnvoll**. **Review-Watch-Item — Full-Rebuild-Restore:** keine Rebuild-Orchestrierung (`run-phase4-apply*.sh` rufen nur `db:apply-override`) zieht `apply:audiobook-narrators` nach — ein voller `db:reset-for-ssot` (TRUNCATE works CASCADE) / `db:seed` löscht die 88 Audio-Rows; der Audio-Apply gehört in die Reset-/Rebuild-Sequenz (kein Blocker — die vom Brief verlangte Re-Apply-Durability ist erfüllt). UI-Pass (Kauf-/Hör-Aktionen + geo-Link-Util) offen.
>
> **2026-05-28 — Audit-Helper + Pilot-Backfills (PR #109 / Brief 103 Daten-Pass).** PR #109 (branch `codex/ingest-batches-audit-pilot`) ist gemergt. Neues read-only Triage-Tool `npm run audit:gap-candidates` (`scripts/audit-gap-candidates.ts`, 246 Zeilen, SQL über `works ⨝ book_details` mit `kind='book' AND format <> 'audio_drama' AND (factions=0 OR locations=0 OR characters=0)`, deterministisch sortiert `external_book_id ASC, slug ASC`, idempotent verifiziert) liefert pro Buch `external_book_id`, `slug`, `title`, `format`, `f/l/c`-Counts + `confidence` plus Summary-Block (`total`, `by domain`, `by axis`); schreibt zusätzlich Markdown-Snapshot nach `ingest/.last-run/audit-gap-candidates.md` für späteren Diff. Kein DB-Write, kein Override-Touch, kein Resolver-Trigger. Helper-Raw-Output: **325 Kandidaten** (HH=73, W40K=252 — ~78 % W40K-konzentriert; Achsen factions=0 → 29, locations=0 → 228, characters=0 → 194). Zwei Pilot-Override-Backfills haben NEW-Range gap_works **18 → 16** gesenkt: HH-0260 *Hunter's Moon* (Russ + Alpharius als `mentioned` im HH-026-Override + neuer `pelago`-Location-Anchor mit `tags=["space_wolves"]` — Post-State `f=2/l=1/c=2`) und HH-0270 *Iron Corpses* (neuer `koparnos`-Character-Anchor mit `primaryFactionId="iron_warriors"`, Override-File unverändert — Resolver greift den bestehenden POV-Eintrag, Post-State `f=1/l=1/c=1`). Reference-Layer post-Daten-Pass: locations `288 → 289` (+pelago), characters `489 → 490` (+koparnos), faction/character/location-aliases unverändert. Story-Chars Tidon/Ven/Sareo/Felbjorn bewusst NICHT promoted (Halluzinations-Schutz / Single-Mention-Bloat-Vermeidung — Brief 103 § Notes). Out-of-Range-Tripwire **= 147** im Pass-15-Config-Lauf (HH-only `applyRange` gegen post-Brief-102 Full-Corpus-DB-Stand — erwartet, Multi-Range-Config wie `consolidation-pass-2.config.json` würde 0 zeigen). Alle Tests grün (`test:resolver` 473/0, `test:resolver-data` 10/0 inkl. neuer coverage-smoke-Check `w40k-001..057 + hh-001..030`, `test:apply-override-dry` `out-of-range=0/unknown-work=0`, `test:collection-refs` 10/0). **Brief-103-Forecast-Korrektur**: Brief-Forecast "~10–20 echte fixable" galt für die post-Triage-Subset (Maintainer-Excel-Sweep-Output), nicht den Helper-Raw-Output. Die rohe Liste ist breiter — viele frühere W40K-Paperbacks (Cain-Reihe `W40K-0012..0019`, Necromunda-Reihe `W40K-0163..0180`, Warhammer-Horror `W40K-0492..0527`) mit `confidence=1.00` haben locations=0 oder characters=0, weil die ursprüngliche Lexicanum-/OL-Crawl-Pipeline für diese Domäne dünner aufgesammelt hat. Pro-Pilot-Effort ~5 min Entscheidung + ~1 min Edit + ~3 min Verify; Skalier-Schätzung 2–4 h für die ~10–20 echten Fixables (Maintainer-Excel-Sweep, kein Brief). **Was Brief 103 noch verbleibt:** der **UI-Pass** (Audio-Drama-Dämpfung im `/buecher?audit=gap`-Bucket + Drift-Tie-Group-Sub-Sortierung nach `raw_name`-Häufigkeit, vollständig in Brief 103 § Constraints/UI + § Acceptance/UI spezifiziert) — läuft als nächster CC-Pass im `chrono-lexicanum-product`-Worktree, Branch z.B. `codex/product-audit-drift-gap-sweep`. Brief 103 selbst trägt `status: implemented` (CC hat geflippt im PR-109-Commit, Brief-103-§-Notes idempotent-Konvention).
>
> **2026-05-27 — Korpus datenkomplett *und* konsolidiert.** PR #108 (Brief 102 / HH-Konsolidierungs-Pass, branch `codex/ingest-batches-hh-consolidation-pass`, 7 Commits c01a795..037b618) ist gemergt: Konsolidierungs-Pass 2 (Full-Corpus W40K 1..57 + HH 1..30) fand 23 Kandidaten-Cluster → **2 Character-Merges** (`merir_astelan→astelan` cross-era lore-deep + `sharrowkyn→nykona_sharrowkyn` same-era mechanical), 21 No-Merges, 0 Flagged. **Cross-era-anchor-breach Aggregator-Tripwire = 0 Treffer** über alle 18 pinned Cross-Era-Surface-Forms → positive Brief-100-ADR-Validation über sechs HH-Wellen. Drei HH-aware Aggregator-Klassen scharf (Slug-edit-distance auf locations / Cross-era-anchor-breach als Re-Annotation auf alias-coincidence-Edges / Primarch-stem auf characters); 1 false-positive locations (`barbarus↔tartarus`) + 1 false-positive characters (`horus↔horus_aximand`), 0 true-positives in den HH-Layern — beide Pass-2-Merges via Pass-1-Base-Heuristiken gefunden. **Counts post-Pass-2-Re-Apply + DB-Sync (859 works):** characters `491 → 489` (-2 mergees), character-aliases `64 → 66` (+2), `work_factions=2754` (+2 re-apply-Drift), `work_locations=1145` (+1), `work_characters=1997` (+5 — re-pointed Junctions auf die zwei Keepers via neue Aliases: `astelan` 5→6, `nykona_sharrowkyn` 1→3), `work_collections=196` / `work_persons=785` / `work_facets=16845` unverändert. Reference factions `202` / locations `288` / facet_values `86` alle unverändert. **`EXPECTED_RANGES.characters.max 2200 → 2500`** (Brief-102-Cap-Re-Tune; ~25% headroom über work_characters=1997, Pass-15-locations-Pattern). `verify-pass.ts`-Out-of-Range-Digest als Brief-102-Bolt-on landed (`if (verify)`-guarded, config-agnostisch — Per-Wave-Resolver-Configs mit verify + Konsolidierungs-Pass-Configs ohne verify); Pass-2-Lauf = **0** (positive Tripwire am datenkomplettem Korpus, exakt das Brief-101-Erfolgs-Pattern). Multi-Range-Loader-Normalisierung (`AggregatorConfig.applyRanges: ApplyRange[]`, Singleton bleibt schreibbar als Backward-Kompat, Hard-Error bei beidem); `run-phase4-apply.sh` expandiert über die Range-Liste. **Brief-094-§-Cadence-Bogen geschlossen** — Pass 2 war der verpflichtende finale Konsolidierungs-Pass nach der HH-Domäne; künftige Konsolidierungs-Pässe sind ad-hoc, nicht mehr im Cadence-Default. Pre-PR-108: PR #107 (Resolver-Loop 5 Wellen, Pässe 11–15, `ssot-hh-003..030`) hatte die HH-Domäne datenkomplett gemacht (DB 585 → 859 = 565 W40K + 294 HH); der headless Resolver-Loop emittiert `all-complete`.
>
> **V2-LLM-Stage de-facto ausgemustert seit Brief 061 (formal ADR 2026-05-15).** Der V1-Code-Pfad bleibt für Reproduzierbarkeit alter Diffs; die V2-Pipeline-Stages 0–2 + 4 leben weiter (Excel-SSOT als Stage 0 ab 057/058, `apply-override.ts` + Resolver-Schicht für Stage 4). Stage 3 (V2-LLM unter `src/lib/ingestion/v2/llm/`) wird **nicht** mehr aufgerufen — eine `claude -p`-Subsession produziert pro 10er-Batch direkt eine `manual-overrides-ssot-w40k-NNN.json` bzw. `ssot-hh-NNN.json`. Begründung und Trade-Off: [`./decisions/why-cc-direct-curation.md`](./decisions/why-cc-direct-curation.md). Resolver-Pässe 1 (063–069) + 2 (072) + 3 (074-impl) + 4 (076-impl) + 5 (089-impl) + 6 (Pass-6-impl) + 7 (Pass-7-impl) haben die Surface-Form-Crystallization für die ersten 450 W40K-Bücher geschlossen (Pässe 4–7 **axis-sliced**, Pässe 6–7 als 100er-Wellen); Brief 077 hat den Grand-Alignment-Junction-Skip im Apply-Layer scharfgeschaltet (`imperium`/`chaos` werden nicht als reguläre Junctions geschrieben, wenn alignment-gleiche Sub-Faction im selben Override-Block resolved ist), Brief 084 den analogen Umbrella-Skip auf der Locations-Achse, Brief 087 die Goodreads-Rating-Beschaffung als vierte Loop-Disziplin. Aktuelle Junction-Counts post-Pass-7-Re-Apply 001..045: `work_factions=1659`, `work_locations=638`, `work_characters=1074`, `work_collections=142`, `work_persons=424` über 450 W40K-Bücher. Brief 090 hat die Resolver-Pass-Maschinerie schlank gemacht (brief-frei + runbook-getrieben, Phase-4 digest-only, stabile wave-parametrisierte Tools); Brief 091 hat Phase 4 in 4a/4b gesplittet + den forward-ref-Guard range-aware gehärtet; Resolver-Pass 7 (`ssot-w40k-036..045`, PR #90) war der erste echte Lauf auf der 6-Phasen-4a/4b-Maschinerie; Pässe 8 + 9 (`ssot-w40k-046..057`, PR #95/#96) haben die letzten zwei Wellen appliziert — **W40K ist datenkomplett: 565/565 W40K-Bücher in der DB** (Junction-Counts post-Pass-9: `work_factions=1903`, `work_locations=733`, `work_characters=1220`, `work_collections=147`, `work_persons=524`; Reference `factions=173`, `locations=225`, `characters=345`). Pässe 8 + 9 liefen supervised über `run-resolver-pass.sh`. [Brief 098](../../sessions/archive/2026-05/2026-05-25-098-arch-w40k-consolidation-pass.md), der W40K-Konsolidierungs-Pass, ist **gemergt** — ein eigener, runbook-getriebener Pass-Typ (Geschwister des Resolver-Passes), der die über die Wellen gewachsene Reference-Schicht global auf Cross-Wave-Dubletten prüft und merged; der Erstlauf über das W40K-Set fand 15 Kandidaten → 2 Merges, Reference-Schicht damit `factions=173` / `locations=224` / `characters=344` (Junction-Counts invariant). Maschinerie-Detail (`consolidation-*`-Skripte, `consolidation-pass-runbook.md`, dedizierte Config) in [`./project-state.md`](./project-state.md) § What's running. Brief 099 (Sessions-Archiv-Sweep, reine Repo-Hygiene) ist ebenfalls gemergt — der `sessions/`-Root ist aufgeräumt. **Brief 100** (HH-Resolver-Domain-Öffnung, gemergt 2026-05-26) hat den headless Resolver-Detektor + Wrapper + Auto-Config + Verify-Trias auf zwei Domänen geöffnet (W40K + HH); drei externe Terminal-Zustände `open-wave | idle | all-complete`, W40K→HH-Übergang als interner Branch-Point; HH-Bootstrap-Welle als `ssot-hh-001..002` (20 Bücher, Pass 10) verbindlich; EXPECTED_RANGES angehoben auf factions/locations/characters 2500/1100/2200; Cross-Era-Identitäten-Disziplin im Runbook §4 + eigener ADR [`./decisions/cross-era-identities.md`](./decisions/cross-era-identities.md). **Brief 101** (HH-Bootstrap Guard-Reason-Split, gemergt 2026-05-26, PR #106) hat den Brief-091 forward-ref Guard in `scripts/apply-override-dry.ts` auf `reason === "unknown-work"` eingeschränkt — `out-of-range`-Constituent-Edges (Constituent steht im Roster, ist nur noch nicht in der kumulativen Apply-Range) sind jetzt informativ im Console-Reason-Breakdown sichtbar (`by reason: out-of-range=N1, unknown-work=N2`, auch bei beiden=0), nur `unknown-work` bleibt scharfer Tripwire. Damit war **Resolver-Pass 10** (`ssot-hh-001..002`, HH-Bootstrap, PR #105) durchführbar — DB **565 → 585** (+20 HH-Bücher HH-0001..HH-0020), JSON-Reference-Schicht +6 factions / +10 locations / +60 characters / +9 aliases (Cross-Era-Disziplin: Luna Wolves→`sons_of_horus`, Mechanicum→`mechanicus`, Imperial Army→`astra_militarum`, Lucius→`lucius_the_eternal`, Ezekyle Abaddon→`abaddon_the_despoiler`), Phase 4b Verify-Digest clean (Smoke-Slugs `tales-of-heresy` 7f/2l/9c + `the-primarchs` 6f/1l/5c, Rating-Coverage 20/20 Goodreads, NEW-Range Audit 20 total / 15 drift_works (Cross-Era-Alias-Resolving wirkt erwartungsgemäß) / 2 gap_works (Data-Findings, kein 4a-Fehler) / 0 content_in_collection — die 3 HH-Anthologien tragen ihre 20 Constituent-Edges out-of-range und materialisieren auf Folge-Wellen via cumulative idempotent re-apply). Out-of-range materialisiert auf Folge-Wellen: `applyCollections` re-evaluiert die Kante idempotent, sobald die spätere Welle den Constituent in die Apply-Range bringt — keine Roster-Maintenance-Edits, kein `collection-gaps.json`-Eintrag nötig (exakt das Erfolgs-Pattern der W40K-Anthology-Forward-Refs aus Pass 6). `work_collections` tickt entsprechend in den Folge-HH-Wellen hoch (HH-0117..HH-0120 / HH-0150..HH-0156 / HH-0157..HH-0165). Die Maschinerie ist damit gegen den Realfall validiert; die Cross-Era-ADR trägt operativ (15/20 drift_works = Aliases resolven auf eine Canonical-Row). **Brief 094 hatte zuvor den Resolver vom SSOT-Loop entkoppelt und beide Loops headless gemacht** — der SSOT-Loop läuft cadence-frei bis 859, der Resolver-Loop verarbeitet kristallisierte Batches in ≤60-Buch-Wellen brief-frei (siehe § Resolver-Loop-Konvention + § What's next).
...
**Brief 100** hat den Detektor + Wrapper + Verify-Trias auf zwei Domänen geöffnet. `CrystallizedBatch` trägt `domain: "w40k" | "hh"`; `DetectInput` zählt Roster pro Domain (`w40kRosterCount` + `hhRosterCount`); `WaveDescriptor` trägt das vollständige `label` (`"ssot-w40k-..."` / `"ssot-hh-..."`); `ApplyRange.domain` erweitert; `parseResolverLoopLog` matcht beide Domain-Heading-Formen; `loadInputs` matcht `manual-overrides-ssot-(w40k|hh)-(\d+)\.json`. `detectNextWave` durchläuft sequenziell W40K vor HH; **drei externe Terminal-Zustände** `open-wave | idle | all-complete` (kein `w40k-complete` mehr — der W40K→HH-Übergang ist interner Branch-Point, materialisiert sich extern als `open-wave` mit der ersten HH-Welle). `run-resolver-loop.sh` domain-agnostisch — `w40k-complete)`-Switch-Case durch `all-complete)` ersetzt, der hardgecodete `WAVE="ssot-w40k-${WAVE}"`-Zusammenbau raus; der Wrapper konsumiert `wave.label` 1:1 aus dem Detektor-Output. **HH-Bootstrap-Wave-Sizing.** Konstanten `HH_BOOTSTRAP_WAVE_TARGET=20` / `HH_BOOTSTRAP_WAVE_HARD_CAP=25` greifen nur bei `hhProgressBatch === 0` (erste HH-Welle = `ssot-hh-001..002`, 20 Bücher; Batch 003 würde gegen die Cap overflowen); spätere HH-Wellen regulär 50/60. Total HH ~6 Wellen (1 Bootstrap + 5 regulär). Pass-Counter global fortlaufend — HH startet bei Pass 10. **Verify-Trias zwei-domänen** (Variant A: statische `{domain, n}`-Tupel mit `as const satisfies ReadonlyArray<…>`); HH-Tupel sind **nicht** pre-added — der Phase-4a-Trigger jeder zukünftigen HH-Welle fordert CC auf, die Tupel der Welle zu appenden. **EXPECTED_RANGES angehoben:** `factions.max 2100→2500`, `locations.max 800→1100`, `characters.max 1400→2200`; Minima unverändert. **W40K-Hardcode-Sweep negativ** (Brief 100 § OQ): `aggregate-surface-forms.ts`, `run-phase4-apply.sh`, `verify-pass.ts`, `run-resolver-pass.sh` tragen alle keine W40K-Konstanten. Live-Smoke gegen den realen Repo-Stand (W40K resolved durch Pass 9, 30/30 HH crystallized) liefert die erwartete Bootstrap-Welle. Tests: `test:resolver-loop-detect` 36 Cases (von 22 prä-100); alle Verify-Tests grün. Erster scharfer headless Mehr-Wellen-Lauf wird der HH-Trial — operativ via `run-resolver-loop.sh`, Philipp triggert. Bekannte, nicht-load-bearing Refinement-Punkte (094-impl § For next session): Resume-Mitigation bei Wrapper-Crash zwischen Phase- und Log-Commit, fehlende Unit-Tests für `resolver-loop-log-update.ts`.

**Cross-Era-Identitäten-Disziplin (Brief 100 § Baustein 3 + Runbook §4 + ADR).** Eine kanonische Identität = eine Canonical-Row, Era-spezifische Bezeichnungen wandern in `*-aliases.json`. Konkret: Faction-Renames (`Luna Wolves` → `sons_of_horus`), Character-Honor-Title-Splits (`Kharn` → `kharn_the_betrayer`, `Abaddon` → `abaddon_the_despoiler`, `Magnus` → `magnus_the_red`, `Lucius` → `lucius_the_eternal`), Primarchen mit Honor-Title als Anker und HH-Surface-Forms als Aliases. Pre-Heresy-Charaktere ohne W40K-Pendant werden frische Canonical-Rows. Ausnahme: echte Identitäts-Disambig → `## Needs decision`-Stop, kein Raten. Surface-Form-Treue bleibt — die Override-Datei trägt die Lore-korrekte Surface-Form, das Resolving zur Canonical-ID via Alias passiert in Phase 4a. Spec im `resolver-pass-runbook.md` §4 (Subsektion „Cross-Era-Identitäten") — selbst-enthalten, ohne Brief-Verweis im operativen Body. ADR [`./decisions/cross-era-identities.md`](./decisions/cross-era-identities.md) trägt Context/Decision/Why/Revisit-Trigger.
...
22. ✅ **Pässe 8 + 9 — W40K datenkomplett (565/565).** Pass 8 (`ssot-w40k-046..051`, PR #95, DB 450 → 510) + Pass 9 (`ssot-w40k-052..057`, PR #96, DB 510 → **565**, finale W40K-Welle). Beide supervised über `run-resolver-pass.sh`. Pass 9 hob `EXPECTED_RANGES.factions.max` 1900→2100 (HH-Headroom-Sizing). Counts post-Pass-9-Re-Apply 001..057: `work_factions=1903`, `work_locations=733`, `work_characters=1220`, `work_collections=147`, `work_persons=524`, `work_facets=11291`.
23. ✅ **Brief 098 — W40K-Konsolidierungs-Pass** (gemergt 2026-05-25). Neuer Pass-Typ (Geschwister des Resolver-Passes) für Cross-Wave-Canonical-Row-Dedup, gebaut + erstmals über das W40K-Entitäten-Set gefahren: 15 Kandidaten → 2 Merges (`vigilus_s`→`vigilus`, `magister_sek`→`anakwanar_sek`), 13 no-merges, 0 geflaggt. Reference `locations 225→224`, `characters 345→344`, `factions 173` unverändert; Junction-Counts invariant. Skripte: `consolidation-aggregate.ts` (deterministischer Aggregator), `consolidation-db-snapshot.ts` (versionierter Pre-Mutation-Snapshot), `consolidation-db-sync.ts` (Dry-Run-Plan + transaktionaler Apply). Spec: `scripts/runbooks/consolidation-pass-runbook.md`. Maintainer-Review-Gate eingehalten. Teil 1 verifizierte `w40k-complete` ohne `resolver-loop-log.md`-Marker-Drift.
24. ✅ **Brief 099 — Sessions-Archiv-Sweep** (gemergt 2026-05-25). Reine Repo-Hygiene: 53 geschlossene Session-Files (062–097) nach `archive/2026-05/`, 188 Pfad-Referenzen in 12 `brain/wiki/**`-Files umgeschrieben, `brain:lint` grün. Kein Code-/DB-Touch.
25. ✅ **Brief 100 — HH-Resolver-Domain-Öffnung** (gemergt 2026-05-26). Detektor + Wrapper + Auto-Config + Verify-Trias zwei-domänen-fähig (W40K + HH); drei externe Terminal-Zustände `open-wave | idle | all-complete`, W40K→HH interner Branch-Point; HH-Bootstrap-Welle `ssot-hh-001..002` (20 Bücher, Pass 10); EXPECTED_RANGES angehoben (2500/1100/2200); Cross-Era-Identitäten-Subsektion im Runbook §4 + neue ADR [`./decisions/cross-era-identities.md`](./decisions/cross-era-identities.md). 36 Detektor-Tests grün; W40K-Hardcode-Sweep über das übrige Pass-Ökosystem negativ. Maschinerie HH-ready.
26. ✅ **Brief 101 — HH-Bootstrap Guard-Reason-Split** (gemergt 2026-05-26, PR #106). `apply-override-dry.ts`-Assertion auf `reason === "unknown-work"`-Subset eingeschränkt; `out-of-range`-Constituent-Edges informativ im Console-Reason-Breakdown sichtbar (`by reason: out-of-range=N1, unknown-work=N2`, auch bei beiden=0); drei Test-Cases (a/b/c) in `test-apply-override-collections.ts` (10/0). `apply-override-collections.ts` unangetastet (Reason-Klassifikation bereits korrekt — die Änderung lebt konsumenten-seitig). Optionen 2 (domain-aware seal) + 3 (allowlist) explizit verworfen. Brief 101 selbst mit `status: open → implemented` im Code-PR mit-portiert.
27. ✅ **Resolver-Pass 10 — HH-Bootstrap-Apply** (gemergt 2026-05-26, PR #105 auf branch `codex/ingest-batches-resolver-trial-hh`). DB **565 → 585** (+20 HH-Bücher HH-0001..HH-0020). Sechs Phasen completed: Phase 1-3 JSON-Reference-Schicht +6 factions / +10 locations / +60 characters / +9 aliases (4 Faction-, 3 Character-, 2 Cross-Era-Honor-Title — Luna Wolves→`sons_of_horus`, Mechanicum→`mechanicus`, Imperial Army→`astra_militarum`, Lucius→`lucius_the_eternal`, Ezekyle Abaddon→`abaddon_the_despoiler`). Phase 4a halt-disziplin-konform auf `## Needs decision` gestoppt (20 HH-Anthology→Constituent-Edges — HH-0020 → HH-0117..HH-0120, HH-0010 → HH-0150..HH-0156, HH-0016 → HH-0157..HH-0165 — brachen den über-strengen Brief-091-Guard auseinander; exakt der Pass-10-Dossier-§7d-Stop-Trigger). Brief 101 hat den Guard gefixt; Re-Run commit `42a6fad` clean (`test:apply-override-dry` `out-of-range=20, unknown-work=0`, `run-phase4-apply.sh` schreibt beide Batches in die DB). Counts PRE→POST: `work_factions 1903→1981`, `work_locations 733→776`, `work_characters 1220→1325`, `work_collections 147 (unverändert — out-of-range)`, `work_persons 524→541`, `work_facets 11291→11672`. Reference factions 173→179 / locations 224→234 / characters 344→404; Aliases faction 59→63, location 16→17, character 43→47. Phase 4b Verify-Digest clean: Smoke-Slugs `tales-of-heresy` (7f/2l/9c) + `the-primarchs` (6f/1l/5c), Rating-Coverage 20/20 Goodreads, NEW-Range Audit 20 total / 15 drift_works (Cross-Era-Alias-Resolving wirkt erwartungsgemäß) / 2 gap_works (Data-Findings) / 0 content_in_collection. Cross-Era-ADR operativ validiert: 15/20 `drift_works` = Surface-Forms resolven auf eine Canonical-Row (Faction-Renames, Character-Honor-Title-Splits, Primarchen-Pattern); `factionsSkippedRedundant` bleibt für Cross-Era-Hits leer (Aliases resolven, kein Skip). Out-of-range materialisiert auf Folge-Wellen via cumulative idempotent re-apply — keine Roster-Maintenance-Edits, kein `collection-gaps.json`-Eintrag.
28. **HH-Resolver-Restwellen** (operativ, kein Brief). `scripts/run-resolver-loop.sh` über die HH-Domäne weiterlaufen lassen — ~5 reguläre Wellen (~60/60/60/50/44 Bücher) bis `all-complete` (274 weitere HH-Bücher nach Pass 10's 20er-Bootstrap). Erster echter headless Mehr-Wellen-Lauf des `run-resolver-loop.sh`-Drivers (Pässe 8/9 + 10 liefen alle supervised). Worauf zu achten ist: jede Welle, die HH-Anthology-Konstituenten (HH-0117..HH-0120 / HH-0150..HH-0156 / HH-0157..HH-0165) in den Re-Apply zieht, sollte `work_collections` proportional hochticken lassen; Cross-Era-Alias-Disziplin bleibt aktiv; EXPECTED_RANGES-Cap-Riss → `## Needs decision`-Stop.
29. **HH-Konsolidierungs-Pass-Brief.** Nach Trial-Abschluss (294/294 HH applied). Schlank, weil Maschinerie aus Brief 098 existiert — der Lauf braucht nur HH-spezifische Kandidaten-Heuristiken. Cross-Wave-Dedup für das HH-Entitäten-Set + Cross-Domain-Dedup für HH-eingebrachte Rows, die mit W40K-Rows reden. Kandidat für Bündelung: `scripts/verify-pass.ts`-Erweiterung um einen `out-of-range`-Count im Verify-Digest post-Apply (Brief 101 § Open questions). **Korpus datenkomplett (859/859)** ist die natürliche Folge.
30. **(Optional) Phase-3-Achsen-Slicing-Folge-Brief.** Nur wenn die HH-Wellen das Signal liefern (Phase 3 driftet in die Dumb-Zone). Pass-10-Bootstrap zeigte keinen Trigger; Folge-Wellen weiter beobachten. In Brief 094 deferred; in Brief 100 § Open questions als Trial-Trigger benannt.
31. **Cockpit-Sub-Sortierung + Public-Rating-Render-Doppelpack — an Brief 096 angedockt.** Innerhalb Drift-Tie-Group sub-sortieren (075/076-impl bestätigen den Bedarf) + `bookDetails.rating` (~576/585 in DB) auf `/buch/[slug]` rendern. UI-leicht; an das laufende Brief-096-Redesign angedockt statt eigener Brief.
32. **OQ3 — Hand-Check-Workflow + Override-Schema.** Cockpit ist verfügbar; Override-Field-Schema + Triage-Disziplin stehen noch aus. Wahrscheinlich gebündelt mit dem Cockpit-Refinement.
33. **Collection-Gap-Ledger-Pflege.** `collection-gaps.json` trägt nach Pass 7 vier Einträge (Green Tide + drei Pass-7-Fälle). Pass 10 hat keinen neuen Eintrag erzeugt (die 20 HH-Anthology→Constituent-Edges sind `out-of-range`, nicht `unknown-work` — also kein Ledger-Bedarf, sie materialisieren auf Folge-Wellen). Wenn der Maintainer-Excel-Workflow die noch-fehlenden W40K-Constituents als eigene Roster-Works modelliert, schließt ein Folge-Brief die Einträge.
34. **3e Voll-Apply** (alle 859 Bücher in DB) sobald die headless Loops durchlaufen — der SSOT-Loop hat 859 kristallisiert; der Resolver-Loop resolved + applied in ≤60-Buch-Wellen. Heute 585/859 (W40K-complete + HH-Bootstrap).
35. **Refresh-Layer für volatile Felder.** Rating-Refresh + Availability + Cover-URL — separater Pfad mit eigener Cadence; der Goodreads-Page-Read aus Brief 087 ist die etablierte Mechanik (Roadmap-Ideas-Backlog „Refresh-Button").
36. **3f Maintenance-Crawler.** GH-Action monthly Wikipedia/TLBranson-Diff für new releases — *post-SSOT* nur noch als Vorschlags-Quelle für die Excel-Workflow-Iteration.
