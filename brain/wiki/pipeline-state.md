...
# Pipeline state (Phase 3, post-100)
...
26. **HH-Resolver-Trial** (operativ, kein Brief). `scripts/run-resolver-loop.sh` über die HH-Domäne, ~6 Wellen bis `all-complete`. Erster echter headless Mehr-Wellen-Lauf des `run-resolver-loop.sh`-Drivers. Worauf zu achten ist: Phase-3-Token-Budget der Bootstrap-Welle (Foundational Ten + Mournival + erste Primarchen = 150–200 neue Characters), Cross-Era-Alias-Disziplin (`Luna Wolves`/`Kharn`/`Abaddon`/`Magnus`/`Lucius` als Aliases zu W40K-Canonical-Rows), EXPECTED_RANGES-Cap-Riss → `## Needs decision`-Stop.
27. **HH-Konsolidierungs-Pass-Brief.** Nach Trial-Abschluss (294/294 HH applied). Schlank, weil Maschinerie aus Brief 098 existiert — der Lauf braucht nur HH-spezifische Kandidaten-Heuristiken. Cross-Wave-Dedup für das HH-Entitäten-Set + Cross-Domain-Dedup für HH-eingebrachte Rows, die mit W40K-Rows reden. **Korpus datenkomplett (859/859)** ist die natürliche Folge.
28. **(Optional) Phase-3-Achsen-Slicing-Folge-Brief.** Nur wenn der HH-Trial das Signal liefert (Phase 3 driftet selbst bei 20-Buch-Bootstrap in die Dumb-Zone). In Brief 094 deferred; in Brief 100 § Open questions als Trial-Trigger benannt.
29. **Cockpit-Sub-Sortierung + Public-Rating-Render-Doppelpack — an Brief 096 angedockt.** Innerhalb Drift-Tie-Group sub-sortieren (075/076-impl bestätigen den Bedarf) + `bookDetails.rating` (~556/565 in DB) auf `/buch/[slug]` rendern. UI-leicht; an das laufende Brief-096-Redesign angedockt statt eigener Brief.
30. **OQ3 — Hand-Check-Workflow + Override-Schema.** Cockpit ist verfügbar; Override-Field-Schema + Triage-Disziplin stehen noch aus. Wahrscheinlich gebündelt mit dem Cockpit-Refinement.
31. **Collection-Gap-Ledger-Pflege.** `collection-gaps.json` trägt nach Pass 7 vier Einträge (Green Tide + drei Pass-7-Fälle). Wenn der Maintainer-Excel-Workflow die fehlenden Constituents als eigene Roster-Works modelliert, schließt ein Folge-Brief die Einträge.
32. **3e Voll-Apply** (alle 859 Bücher in DB) sobald die headless Loops durchlaufen — der SSOT-Loop kristallisiert bis 859, der Resolver-Loop resolved + applied in ≤60-Buch-Wellen.
33. **Refresh-Layer für volatile Felder.** Rating-Refresh + Availability + Cover-URL — separater Pfad mit eigener Cadence; der Goodreads-Page-Read aus Brief 087 ist die etablierte Mechanik (Roadmap-Ideas-Backlog „Refresh-Button").
34. **3f Maintenance-Crawler.** GH-Action monthly Wikipedia/TLBranson-Diff für new releases — *post-SSOT* nur noch als Vorschlags-Quelle für die Excel-Workflow-Iteration.
