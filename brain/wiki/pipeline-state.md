...
# Pipeline state (Phase 3, post-PR-107 / Pässe 11–15, Korpus datenkomplett 859/859)
...
>
> **2026-05-27 — Korpus datenkomplett.** PR #107 (Resolver-Loop 5 Wellen, Pässe 11–15, `ssot-hh-003..030`) ist gemergt: **DB 585 → 859** (+274 HH-Bücher HH-0021..HH-0294); die HH-Domäne ist damit datenkomplett (294/294), parallel zum W40K-565-Seal. Der headless Resolver-Loop emittiert `all-complete` — fünf clean Two-Domain-Pässe in Folge ohne `## Needs decision`-Stop und ohne facetId-Strips, exakt der Brief-100-Forecast (60/60/60/50/44 Bücher). **Counts post-Pass-15-Re-Apply `001..057 + hh-001..030`:** `works=859`, `work_factions=2752`, `work_locations=1144`, `work_characters=1992`, `work_collections=196`, `work_persons=785`, `work_facets=16845`. Reference `factions=202` (+23 seit Pass 10) / `locations=288` (+54) / `characters=491` (+87) / `facet_values=86`. `persons.json` bleibt 100, DB-persons via `ensurePersonsExist` 541→785 (+244 HH-Author-Slugs aus dem audio-drama-lastigen HH-Mittelfeld + ssot-hh-030 artbook/scriptbook tail mit `author:"?"`). EXPECTED_RANGES.locations.max in Pass 15 1100 → 1500 (Pass-14-forecasted Re-Tune; ~24% post-bump headroom); factions cap `3200` (2754 ≈ 14% headroom) und characters cap `2200` (1992 ≈ 9% headroom — der tightest bound) ohne Re-Tune. Cross-Era-ADR über den vollen HH-Korpus operativ gehärtet — `factionsSkippedRedundant`-Bucket bleibt für Cross-Era-Hits leer, der Brief-100-§4-Entwurf trägt. **Deferred-Edge-Pattern aus Brief 101 trägt:** OLD-Range HH-0001..HH-0250 cumulative `content_in_collection=49` nach Pass 15 (`work_collections` 147→196 über die Folge-Wellen — die Pass-10-Anthology-Constituent-Edges sind idempotent materialisiert). Nächster Architekten-Brief: **HH-Konsolidierungs-Pass** (schlank — Maschinerie aus Brief 098 existiert; potenzieller Bündel-Kandidat: Brief 101 § Open questions `verify-pass.ts`-Out-of-Range-Count).
>
