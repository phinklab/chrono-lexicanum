/**
 * aggregate-surface-forms-089.ts — Phase 0 helper for Resolver-Pass 5 (Brief 089).
 *
 * Reads `manual-overrides-ssot-w40k-021..025.json` (50 books, IDs
 * `W40K-0201..W40K-0250`) plus the current resolver seed-data + alias tables,
 * aggregates surface forms per axis (factions / locations / characters) with
 * frequency + example books, resolves each surface form against the
 * pre-Pass-5 resolver state (direct → alias → unresolved, exactly mirroring
 * `src/lib/resolver/index.ts`), and emits a deterministic markdown report on
 * stdout — the input that gets folded into the Phase-0 dossier under
 * `sessions/resolver-dossiers/2026-05-21-089-resolver-pass-5-dossier.md`.
 *
 * Read-only; no DB, no LLM, no network. Idempotent — re-running on the same
 * inputs yields byte-identical output.
 *
 * Usage:
 *   npx tsx scripts/aggregate-surface-forms-089.ts > /tmp/agg-089.md
 *   (or piped to less / wc / diff against a prior run)
 *
 * Pattern analogue: Brief 076 `aggregate-surface-forms-076.ts`.
 */
...
const overrideFiles: OverrideFile[] = [21, 22, 23, 24, 25].map(
  (n) =>
    JSON.parse(
      readFileSync(
        join(SEED, `manual-overrides-ssot-w40k-0${n}.json`),
        "utf8",
      ),
    ) as OverrideFile,
);
...
const CLUSTER_BY_BATCH: Record<string, string> = {
  "ssot-w40k-021": "calpurnia/templars/dow",
  "ssot-w40k-022": "grey-knights/blood-angels",
  "ssot-w40k-023": "space-hulk/guard-open",
  "ssot-w40k-024": "astra-militarum-main",
  "ssot-w40k-025": "am-close/deathwatch",
};
...
const out: string[] = [];
out.push("# Aggregator output — Resolver-Pass 5 (Brief 089) Phase 0");
out.push("");
out.push("Generated by `scripts/aggregate-surface-forms-089.ts`. Inputs: 5 override files (`ssot-w40k-021..025`, 50 books W40K-0201..W40K-0250) + `book-roster.json` + the current `factions.json` / `locations.json` / `characters.json` + their alias tables.");
out.push("");
out.push("Resolver-Baseline (Reference rows + aliases, pre-Pass-5):");
...
out.push("");
out.push("## Book table (50 entries)");
out.push("");
