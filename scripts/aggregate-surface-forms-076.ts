/**
 * aggregate-surface-forms-076.ts — Phase 0 helper for Resolver-Pass 4 (Brief 076).
 *
 * Reads `manual-overrides-ssot-w40k-016..020.json` (50 books, IDs
 * `W40K-0151..W40K-0200`) plus the current resolver seed-data + alias tables,
 ...
 * pre-Pass-4 resolver state (direct → alias → unresolved, exactly mirroring
 * `src/lib/resolver/index.ts`), and emits a deterministic markdown report on
 * stdout — the input that gets copy-pasted into the Phase-0 dossier under
 * `sessions/resolver-dossiers/2026-05-16-076-resolver-pass-4-dossier.md`.
 *
 * Read-only; no DB, no LLM, no network. Idempotent — re-running on the same
 * inputs yields byte-identical output.
 *
 * Usage:
 *   npx tsx scripts/aggregate-surface-forms-076.ts > /tmp/agg.md
 *   (or piped to less / wc / diff against a prior run)
 *
 * Pattern analogue: 074-impl `strip-unknown-facets-074.ts`.
 */
...
const overrideFiles: OverrideFile[] = [16, 17, 18, 19, 20].map(
  (n) =>
    JSON.parse(
      readFileSync(
...
const CLUSTER_BY_BATCH: Record<string, string> = {
  "ssot-w40k-016": "space-wolves",
  "ssot-w40k-017": "necromunda-classic",
  "ssot-w40k-018": "necromunda-modern",
  "ssot-w40k-019": "lc/gw/sd-open",
  "ssot-w40k-020": "sd-cont/calpurnia",
};
...
const out: string[] = [];
out.push("# Aggregator output — Resolver-Pass 4 (Brief 076) Phase 0");
out.push("");
out.push("Generated by `scripts/aggregate-surface-forms-076.ts`. Inputs: 5 override files (`ssot-w40k-016..020`, 50 books W40K-0151..W40K-0200) + `book-roster.json` + the current `factions.json` / `locations.json` / `characters.json` + their alias tables.");
out.push("");
out.push("Resolver-Baseline (Reference rows + aliases, pre-Pass-4):");
