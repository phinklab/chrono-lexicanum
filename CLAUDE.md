...
> **Lies nur, was die Aufgabe braucht.** Die Liste oben ist das *Maximum* für eine normale Planungs-/Architektur-Session. Schritte 4–5 (`project-state.md` + `open-questions.md`) sind die synthetisierten „Where are we now"-Seiten — sie werden **bewusst schlank** gehalten (nur aktueller Stand; Historie → `log.md` + git), damit dieser Session-Start-Floor klein bleibt. Eine **enge oder Strang-Session** (ein einzelner UI-Tweak, ein Resolver-Fix, eine Doc-Änderung) braucht die volle Synthese **nicht**: lies CLAUDE.md + `brain/CLAUDE.md` + den konkreten Brief und die darin genannten Files, und überfliege oder überspringe Schritte 4–5. Jeder am Session-Start gelesene Token fehlt der eigentlichen Arbeit — eine Plan-Mode, die vorab alles liest, hat 100k+ verbrannt, bevor sie anfängt. Im Zweifel weniger lesen und die spezifische Seite erst ziehen, wenn die Aufgabe sie wirklich braucht.

> **SSOT-Loop-Iteration?** Eine Loop-Iteration (`scripts/run-ssot-loop.sh`) ist ein mechanischer Task, **keine** normale Session: folge [`scripts/runbooks/ssot-loop-runbook.md`](./scripts/runbooks/ssot-loop-runbook.md) und überspringe diese Session-Start-Leseroutine (Brief 061 selbst wird **nicht** gelesen).

> **Resolver-Welle?** Eine Welle — headless via `scripts/run-resolver-loop.sh` oder einzeln via `scripts/run-resolver-pass.sh <config>` — ist ein mechanischer Task, **keine** normale Session: folge [`scripts/runbooks/resolver-pass-runbook.md`](./scripts/runbooks/resolver-pass-runbook.md) + der per-Welle-Config und überspringe diese Session-Start-Leseroutine. **Lies keinen Brief** — weder einen per-pass Architect-Brief (existiert seit Brief 094 nicht mehr) noch den „höchsten offenen Brief". Operative Spec ist ausschließlich das Runbook; Herkunft der Rationale steht im Runbook-Anhang.

> **Consolidation-Pass?** Ein Konsolidierungs-Pass — phase-weise gefahren über `scripts/consolidation-aggregate.ts`, `scripts/consolidation-db-snapshot.ts`, `scripts/consolidation-db-sync.ts` und den geteilten `scripts/run-phase4-apply.sh scripts/consolidation-pass.config.json` — ist ein mechanischer Task, **keine** normale Session: folge [`scripts/runbooks/consolidation-pass-runbook.md`](./scripts/runbooks/consolidation-pass-runbook.md) + der dedizierten `scripts/consolidation-pass.config.json` und überspringe diese Session-Start-Leseroutine. **Lies keinen Brief** — weder Brief 094 noch Brief 098. Operative Spec ist ausschließlich das Runbook; Herkunft der Rationale steht im Runbook-Anhang.
...
**Temporary launch exception (decided 2026-07-10).** While the maintainer's serial launch programme is active, its sessions run one at a time from the coordination worktree `C:\Users\Phil\chrono-lexicanum`, even when the logical task strand is Product or Batches. This suspends **worktree routing only**: derive and announce the logical strand from the explicit launch prompt, use its branch prefix, keep each PR's contents strand-pure, keep `brain/**` + `sessions/README.md` in separate coordination PRs, and do not run parallel launch sessions. Kickoff may use the maintainer-local launch prompt collection instead of a per-session architect brief; implementer reports remain mandatory. Normal three-worktree routing resumes after launch/post-launch cleanup or as soon as parallel work restarts. Durable rationale: [`brain/wiki/decisions/launch-single-worktree-mode.md`](./brain/wiki/decisions/launch-single-worktree-mode.md).

At the start of any implementation session:

1. Run `git branch --show-current` and `git status --short --branch`.
2. Infer the strand from the current worktree path. CC always derives the strand itself — never ask the maintainer which worktree this is. During the temporary launch exception above, infer the **logical** strand from the explicit launch prompt while the physical worktree stays Coordination.
