...
- **E4 — Content-Release ist zweistufig:** Content-Freeze → `db:sync` vollständig grün → Snapshot auf frischem Batches-Branch regenerieren → Manifest/Counts/Hashes/Diff prüfen → **der Snapshot-PR ist der Production-Deploy** → danach Revalidation + Live-Smoke. Gilt für den Launch und jedes spätere Content-Release. Der v1-Satz „Regen in Apply/Sync verdrahten, damit kein Deploy mit altem Stand baut" war falsch herum: ein Daten-Merge deployt *vor* dem Post-Merge-Sync; „heilt sich via ISR" ist kein Releasevertrag und hilft z. B. einer beim Build erzeugten Sitemap nicht.
...
**Hinweis:** persistenter Archive-Cache erst in S6 (nach der Payload-Diät; 2-MB-Data-Cache-Limit). Der Apply-seitige Hook-Aufruf ist S3a (Batches).

### Session 3a — DB-Rollen, Migrations-Rehearsal, Apply-Hook  — Größe M · Strang: Batches (+ `.github`)
...
3. **Credential-Trennung:** Vercel besitzt nur `RUNTIME_DATABASE_URL`; der Migrations-Workflow besitzt `MIGRATION_DATABASE_URL`; lokale Apply-/Ingest-Skripte nutzen weiterhin bewusst das privilegierte Credential.
...
5. **Apply-/Sync-Hook:** **genau ein** Revalidation-POST nach **vollständig erfolgreichem** `db:sync` (nicht pro Teilschritt; derzeit ruft kein Skript den Endpoint, der Route-Docstring behauptet es). Ziel aus `REVALIDATE_BASE_URL` (Script-Env), mit Timeout, Statusprüfung und klarer Recovery-Meldung bei Fehlschlag (manueller curl steht im Runbook).
...
**Fertig wenn:** Negativtests grün (submissions unlesbar, DDL/Katalog-DML verweigert) · Rehearsal migriert ein frisches Postgres und der zweite Lauf ist idempotent · Vercel-Env enthält nur die Runtime-URL · ein `db:sync`-Lauf endet mit genau einem erfolgreichen, geloggten Revalidation-POST.

### Session 3b — CSP, Login, Health, Audio  — Größe S–M · Strang: Product
...
**Fertig wenn:** Prod-CSP ohne `unsafe-eval` · Login-Vergleich timing-safe · Audio spielt aus dem Public-Bucket (200/206 + Range belegt) · Healthcheck-Verhalten dokumentiert.
...
2. **`/api/revalidate` mitziehen:** die `ENTITY_ROUTES`-Pfadliste ist hart deutsch kodiert (`route.ts:48-53`) — Teil **dieser** Session, sonst purgt der Apply-Hook nach der Migration ins Leere.
...
6. Vercel-Env vollständig (`SITE_URL`, `RUNTIME_DATABASE_URL`, `REVALIDATE_TOKEN`, …), Deployment Protection, Preview-Gate-Zustand.
...
**Sequenz:** S0 → S1a → S1b → S2 → S3a → S3b → S4 → S4b (Mini: Buch-Projektion in den Snapshot) → S5 → S6 → S7a → S8 → S9 → S10a (→ S10b nur bei Messbefund) → **Launch-Readiness** → Launch → PL1, S7b, S11.

| Stufe | Schritte |
|---|---|
| **Vor allem anderen** | S0 (Plan kanonisieren, Briefstatus, URL-/Domain-/Era-/Observability-Entscheidungen) |
| **Muss vor Gate-off** | S1a, S1b, S2, S3a, S3b, S4 (+ S4b-Mini), S5, S6, S7a, S8, S9, S10a |
...
9. **S2 schlank** — data/null/throw-Konvention statt Result-Framework; Tags für entities/timeline/podcasts; Apply-Hook mit `REVALIDATE_BASE_URL`, Timeout, Statusprüfung, genau ein POST nach grünem Sync (nach S3a verschoben — Batches-Pfade).
10. **Revalidation-Semantik** — gegen die Next-16-Doku aufgelöst: Codex hatte recht, `revalidateTag(tag,"max")` ist **stale-while-revalidate**; der Kommentar in `route.ts:99-101` („hard-purge") ist falsch. Für den Apply-Hook wird `{ expire: 0 }` (dokumentiertes Route-Handler-Pattern) empfohlen; Entscheidung + Benennung im S2-Vertrag.
