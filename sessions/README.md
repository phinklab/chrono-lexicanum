...
| [113-arch-entity-graph-panel](./2026-06-01-113-arch-entity-graph-panel.md) | architect | open | **Entity-Graph Step 2 — EntityView-Redesign + In-Context-Panel (Product), phasiert.** Phase A: geteiltes `EntityView` aufs Design-Mockup umrüsten (Header-Meta aus vorhandenen Fakten, Werk-Karten je Kind, rechte VERKNÜPFT-Rail mit Reflow, Gradient-to-dark; kein Bild, keine neuen Daten). Phase B: In-Context-Panel (intercepting routes, reused `loadEntity`+`EntityView`, zero fork, gleiche URL, SSG-Fallback). Ein PR (A+B zusammen), `/clear` zwischen den Phasen, kein Zwischen-Merge; Optik an CC (Design-freedom). |
