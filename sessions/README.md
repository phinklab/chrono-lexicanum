...
| [111-arch-session-start-token-diet](./2026-06-01-111-arch-session-start-token-diet.md) | architect | open | **Session-start token diet + archive-sweep (eingedampft 2026-06-02).** Brain-Slim + Brief-Disziplin + Read-Order-Regel bereits angewandt (Floor ~91k → ~18k). CC-Handoff = nur noch die doc-only-Teilmenge: Archiv der 7 ungekoppelten Paare (098–106) + NNN-098/099-Fix → 115/116 + Ref-Rewrites, doc-only → `main`. Logs/Runbooks-Relocation war script-gekoppelt → ausgelagert nach **Brief 117**. |
