...
| [111-arch-session-start-token-diet](./2026-06-01-111-arch-session-start-token-diet.md) | architect | open | **Session-start token diet + archive-sweep.** Brain-Slim (project-state/open-questions/README) + Brief-Disziplin + Read-Order-Regel sind bereits direkt angewandt (Floor ~91k → ~18k Token). CC-Handoff = physischer `sessions/`-Archiv-Sweep + NNN-098/099-Fix + Logs/Runbooks-Relocation + Ref-Rewrites, `brain:lint`-grün, doc-only → `main`. |
