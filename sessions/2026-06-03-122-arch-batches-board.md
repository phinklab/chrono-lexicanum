...
| B6 | Dead-Code-Retirement: V1-Ingestion + V2-LLM + V2-Rest ausmustern (Carve-out post-171 erweitert: der komplette `equiv:diff`-Golden-Pfad bleibt, nicht nur der Excel-Loader); stale `CLAUDE.md`-Stack-Tabelle fixen | ☑ erledigt via Brief 177 / PR #214 (43 Dateien, −8.994 LOC; Golden-/Refusal-Pfade bleiben) |
| B7 | brain:lint always-read Budget-Guardrail (Spec: archivierter Brief 112) | ☑ erledigt via Brief 176 / PR #212 |
...
| B10 | Weekly content refresh: wöchentl. Cron difft Track of Words (Bücher) + Registry-Podcast-Feeds gegen committeten Bestand → Vorschlag-Report + Apply-File → PR-Approval → bestehende Apply-Pfade. Additions-only, kein DB-Write aus CI. Spec [133](./archive/2026-06/2026-06-09-133-arch-weekly-content-refresh.md) | ☑ erledigt; W28 (#230/#231) belegt den lebenden Flow: 2 Bücher ignoriert, +10 Episoden appliziert, Cursor fortgeschrieben |
| B11 | Großer Buch-Reviewer: Multi-Agent-Review (Finder + adversariale Verifier, Muster der Deep-Reviews 140/141/144) über alle 889 Bücher — Facets, Factions, Junctions, Synopsis-Qualität; Findings landen in der 149er-Review-Queue. **Durch B2/149 entsperrt** | ☑ erledigt (154 Voll-Lauf 889 Bücher: 639→608 bestätigt, 96 promoviert + Drukhari-Split; Stage 3 155 Web-Enrichment; Gate F/L 158 → +20 Factions / +142 Locations in der Prod-DB) |
| B12 | Ask-Logik-Tuning: `recommend()`-Gewichte + Curation-Rules überarbeiten („beste Optionen erscheinen nicht immer"); Werkzeug: 1080er-Audit (`npm run audit:ask-combinations`). **Nach B11** (Datenqualität zuerst, sonst wird gegen falsche Facets getuned) | ☑ erledigt (Brief 164 Ask-Overhaul: Contract-Reduktion 5→4 Fragen, lane-skopiertes Anker-Signal, Precompute-Matrix; gemergt + gepullt, Impl-Report 2026-06-26 nachgetragen) |
| B13 | Weekly-Refresh-Hardening / operativer Preflight: PR-Prompt im Report, Podcast-Artefakt-↔-DB-Drift-Audit (read-only, lokal), Cursor-Lifecycle bootstrappen (`curation-state.json`/`book-seen.json` committen), Outage≠Ruhe (Degraded schließt PR nicht). Detection-only, kein CI-DB-Write. Spec [151](./archive/2026-06/2026-06-14-151-arch-weekly-refresh-hardening.md). **Vor B2/149** | ☑ erledigt (PR #175; 4 Tasks, 65 Tests; `REFRESH_RESULT=degraded`, Cursor-Baseline `2026-01-01`, read-only `refresh:audit-artifacts`. Maintainer-Preflight: einmal `npm run refresh:audit-artifacts` lokal) |
| B14 | Local-only Curation Admin Tool: separates lokales Browser-Tool für Hand-Kuration auf dem 149er-Apply-Pfad. **Verworfen am 2026-06-17 nach Maintainer-Test:** JSON-Editor-UI passt nicht zu Philipps Workflow und soll nicht weiterverfolgt werden. Hand-Kuration läuft künftig per normalem Codex-Auftrag: Maintainer beschreibt die Korrektur in Prosa, Codex ändert `curation-overlay.json` und verifiziert per Dry-Run/Verify. | ☒ verworfen |
