...
4. **Green-Tide-Stresstest in der Erst-Fassung nicht erfüllbar.** Drei Bugs übereinander:
   ...
   **Cowork-Entscheidung:** Green-Tide-Stresstest ist für 074 **out of scope**. Stattdessen: CC reportet Pre-/Post-Apply-Count für `work_collections` global, plus separaten Status-Bullet für The Green Tide (erwartet: 0 zusätzliche Rows wegen Roster-Lücke; CC bestätigt im Report + listet als Hand-off-Punkt für einen Cowork-Folge-Mini-Brief, der den Roster-Patch macht). Falls die Override-Files für 011..015 noch andere Omnibus-Entries enthalten, deren Constituents tatsächlich im Roster-`collections` stehen, ist DAS der reale Cross-Batch-Stresstest in dieser Welle — CC darf den entdecken und dokumentieren.
...
**Cross-Batch-Continuity-Status.** Die dritte Welle bringt drei completed-trilogy-arcs in die Authority-Schicht: Cawl-Trilogy (`The Great Work` 011 → `Genefather` 014 → `Archmagos` 015), Watson-Inquisition-War-Trilogy (`Inquisitor / Draco` + `Harlequin` + `Chaos Child` alle in 015 en bloc), plus Cadian-Legacy-Mini-Arc innerhalb 014 (`Longshot` → `Creed: Ashes of Cadia` → `Fall of Cadia`). `The Green Tide` (015) ist ein Omnibus, dessen Constituents im Lore in 012 (`Catachan Devil`, W40K-0118) und 013 (`Warboss`, W40K-0128) leben — aber **im `book-roster.json` heute nicht als Constituents erfasst** (siehe Erratum-Punkt 4: Roster-Note „individual contents not explicitly recoverable", 0 `roster.collections`-Rows mit W40K-0147 als parent). Der 072-`applyCollections.external_book_id`-Refactor ist also in dieser Welle nicht über Green Tide testbar; CC dokumentiert den Pre-/Post-Apply-`work_collections`-Count und listet etwaige andere Omnibus-Entries mit echten Roster-Rows. Plus mehrere named-vehicle-cross-batch-Continuities (Steel Tread 012 → Demolisher 015, Ithraca's Vengeance 011-internal).

**Roster-Cleanup-Lücken sichtbar.** Loop-Log dokumentiert 5 `data_conflict` flags für missing roster authors (W40K-0141 Mike Brooks / W40K-0142 Jonathan D. Beer / W40K-0143 Robbie MacNiven / W40K-0146 Rhuairidh James / W40K-0147 multi-author Green-Tide-Omnibus). Diese sind **out of scope** für diesen Brief — Roster-Cleanup ist Maintainer-Excel-Workflow per Brief-061-Konvention; der Resolver-Pass touched nur die ressorted `seed-data`-JSONs.
...
- Status-Pass für `work_collections` (global): CC reportet Pre-/Post-Apply-Count und dokumentiert separate den Status für The Green Tide (W40K-0147) — Erwartung: **0 zusätzliche Rows** wegen `roster.collections`-Lücke (siehe Erratum-Punkt 4: Roster-Note „individual contents not explicitly recoverable"; `applyCollections` zieht ausschließlich aus `roster.collections`, `scripts/apply-override.ts:796`-Bereich). Falls 011..015 andere Omnibus-Entries enthalten, deren Constituents echte `roster.collections`-Rows haben, ist DAS der reale Cross-Batch-Stresstest in dieser Welle — CC darf den entdecken und dokumentieren, statt ihn vorzuschreiben.
...
- `the-green-tide` (015) — Omnibus mit Roster-Lücke (siehe Erratum-Punkt 4); CC verifiziert die 0 zusätzlichen `work_collections`-Rows im Smoke
...
- **Roster-Cleanup.** Die 5 `data_conflict`-Author-Lücken aus 011/015 bleiben im Excel-Maintainer-Workflow; CC touched `book-roster.json` nicht und führt `npm run import:ssot-roster` nicht aus.
...
- **Roster-Patch für Omnibus-Constituents** (Hand-off, neu post-Codex-Review). The Green Tide (W40K-0147) hat im Roster die Note „individual contents not explicitly recoverable" und 0 `roster.collections`-Rows mit W40K-0147 als parent. Falls 011..015 weitere Omnibus-Entries mit ähnlicher Lücke enthalten (CC's Aggregation), im Report listen — Cowork zieht einen Roster-Patch-Mini-Brief daraus.
