...
3. **Router-Prefetches** (Response-Watcher-Ignore, eng: nur Requests mit `Next-Router-Prefetch`/`purpose: prefetch`-Header): Produktions-`<Link>`s prefetchen sichtbare Ziele, und auf dem No-DB-Server dürfen die dynamischen Ziele (/timeline, /archive, /compendium, /person/…) mit 5xx antworten — das ist die designte Degradation der Zielroute, kein Defekt der getesteten Seite (die pinnt der Degraded-Smoke bei echter Navigation).
4. **UA-Konsolen-Echos von Netzwerkfehlern** (`Failed to load resource: …`): für Netzwerkfehler ist der Response-Watcher die Autorität (Status, Pfad, Prefetch-Header); das Konsolen-Echo trägt in CI-Chromium keine verlässliche Quell-URL und ließe sich entschuldigten Prefetches nicht zuordnen. Nicht-Netzwerk-`console.error` bleibt harter Fail.
...
- **CI-Härtung nach zwei roten PR-Läufen (Hauptursache erst in Lauf 2 sichtbar):** (a) **`SITE_URL` fehlte den Smoke-Servern zur Laufzeit** — der S5-Guard wirft auch im Runtime, CI hat kein `.env.local`; jede dynamisch gerenderte Route (inkl. `/ask` und aller Prefetch-Ziele) antwortete 500. Fix: beide webServer-Envs setzen den Reserved-Invalid-Host des CI-Builds. (b) Link-**Prefetches** dynamischer Routen dürfen auf dem No-DB-Server 5xx sein (→ Ausnahme 3) und **Netzwerk-Konsolen-Echos** gehören dem Response-Watcher (→ Ausnahme 4). (c) Degraded-Test akzeptiert **beide** Degradationsmodi (Befund 1: Pre-/Post-Shell) statt nur des lokal beobachteten. (d) Menü-Fokus-Test deterministisch verankert (expliziter `burger.focus()` — Klick-Fokus ist UA-abhängig, einmal lokal geflaked). (e) Live-Projekt `timeout: 180s` (Erst-Render von /timeline zahlt seine DB-Roundtrips bei WAN-Latenz; an langsamen Abenden >60s, danach cached-instant; Prod zahlt sie intra-region). Danach lokal erneut: 15/15 + 6/6.
