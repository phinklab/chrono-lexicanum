...
Everything about how these pages *look and feel* is yours: layout, composition, spacing, type, colour usage within the existing token system, motion, micro-copy/voice, the visual treatment of each section, and all component class shapes. This brief specifies *what information appears, how it is grouped into sections, and how it is wired to data and routes* — the grammar — never the appearance. Where it names sections ("header", "related works grouped by media kind", "cross-link rail"), read that as information architecture, not a layout spec; if a section's best form is a tab, a rail, an accordion or a grid, that's your call. Build under the project's CSS pattern (see Constraints — ownership-split files under `src/app/styles/`, shared design tokens), and reuse the existing `chrono/*` primitives where they fit.

**Visual reference & quality bar.** Build these pages to read as one family with the project's *reworked* surfaces — the Chronicle/timeline, the reworked Ask funnel, and the `/buecher` list — and match that level of polish. Those redesigns are the bar, **not** the old stub/legacy shells: the same chrome, card/section language, typographic rhythm and restraint. Treat this as a coherence pointer, not a layout spec — study those surfaces, then make the entity hubs feel like they belong beside them rather than inventing a parallel look. (File pointers in Notes.)
...
**Scope is exactly those three routes.** Swapping the three stub shells for real entity-hub pages is the whole job. Removing the stub code *for these three routes* is expected; a wider cleanup is not — no reorg or dedupe of the existing `src/app/styles/**`, no dead-CSS/dead-JS sweep, no `/buch` rebuild, no search / panel / podcasts. See Out of scope.
...
**Routing identity (verified against `src/db/schema.ts`):** the `factions` / `characters` / `locations` reference tables key on a **string primary key** (e.g. `horus`, `thousand_sons`, `prospero`). They have **no `slug` column** — only `works` does (the `varchar("slug")` at schema L214, which powers `/buch`). The existing route folders are *named* `[slug]`, but for these three reference routes the segment value is that canonical string id (the current stub even comments `/charakter/horus`). Treat the `[slug]` param as the id; **do not** add a slug column, a migration, or a slugify step — that is unnecessary and out of scope (see Constraints).
...
- **The `[slug]` segment is the canonical string id — do not build a slug system.** Load each page by the reference table's string primary key; `generateStaticParams` enumerates those existing ids. No slug column, no migration, no slugify for reference entities. The folder stays named `[slug]`; renaming the segment is not in scope.
- **Canonical type→route map (fixed by the existing routes):** character → `/charakter/[id]`, faction → `/fraktion/[id]`, location (world / sector) → `/welt/[id]`. Step 3's search will hard-code this map, so keep these segments stable. (Only the *optional* era spelling — `/aera` vs `/ära` — is still open; see open questions. Era pages are not required this step.)
- **Cross-links navigate the graph.** Each page links to its neighbour entities (faction ↔ characters ↔ locations, etc.) by canonical `/{typ}/[id]` URL (the map above). Those URLs are the shareable contract and must stay stable.
- **Async params (Next 15)** — `await` `params` / `searchParams`.
- **CSS pattern — ownership-split globals (match what's on `main`).** The site's CSS is organised as ownership files under `src/app/styles/` (`00-tokens.css` … `58-ask-booklist.css`), each `@import`ed from `globals.css` in cascade order; `globals.css` itself is import-only. There are **no CSS Modules** in the codebase — do **not** introduce them. Put the entity pages' styles in a dedicated ownership file (a new page-level file in the 50-series, e.g. `59-entity.css`, or by repurposing `52-stub-shell.css` since the stubs are being replaced — your call), `@import`ed from `globals.css`. Reuse the design tokens already in `00-tokens.css`; promote a repeated colour/size to a token there before hard-coding it. Keep `globals.css` import-only.
- **Accessibility floor (not the full a11y arc):** semantic heading order, real landmarks, keyboard-reachable links. The deep a11y / reduced-motion pass is a later concern — here, simply do not introduce motion that ignores `prefers-reduced-motion`.
...
- **Search** (typeahead, `/suche`) → Step 3 — and with it the Brief-104 alias resolver, built then; Step 1 does **no** alias matching, it loads each page by canonical id. **Browse / filter** (`/werke`) → Step 4. **Homepage** → Step 5. **Panel / overlay + intercepting routes** → Step 2.
...
- **The CSS modernization cleanup** — don't touch the existing `src/app/styles/**` files (palette dedupe, tokenizing, reorganizing) or `globals.css` beyond adding your one `@import`. Add the entity pages' own ownership file; leave the rest alone. No visual-regression tooling — see Acceptance (eyeball the preview).
- **A slug system for reference entities** — no slug column, migration, or slugify (Context / Constraints).
- **General cleanup beyond the three routes.** Deleting the three stub shells' code is expected; touching anything outside `/charakter`, `/fraktion`, `/welt` (and their new shared modules) is not.
...
- [ ] `/charakter/[slug]`, `/fraktion/[slug]`, `/welt/[slug]` (folder named `[slug]`, segment value = canonical entity id) render real data for seeded ids — header, type-specific facts, related works grouped by media kind, and a cross-link rail — fully replacing the stub shells.
...
- [ ] Cross-links navigate between entity pages; the `/{typ}/[id]` URLs (folder `[slug]`, value = canonical id) are stable.
- [ ] The entity pages' CSS lives in a dedicated `src/app/styles/` ownership file (reusing `00-tokens.css` tokens); `git diff` shows `globals.css` gained at most one `@import` and **no** component rules, and no `.module.css` was introduced.
- [ ] **Browser smoke (the safety net for this project):** the three new routes render correctly on the dev server / Vercel preview at desktop **and** mobile width — layout intact, links reachable — noted in the report. (No Playwright / visual-regression suite — that arc is dropped; see Brief 108's 2026-06-01 redirect.)
...
- The three core mappings are pinned (character → `/charakter`, faction → `/fraktion`, location → `/welt`, matching the existing routes). Only the **optional era spelling** (`/aera` vs `/ära`) and any later `/welt` vs `/ort` reconsideration are still open — flag a preference before Step 3 hard-codes the full map, but nothing here blocks Step 1.
...
- References: schema `src/db/schema.ts`; stubs `src/app/{charakter,fraktion,welt}/[slug]/page.tsx`; the CSS pattern = ownership-split files under `src/app/styles/` (tokens in `00-tokens.css`; current stub styling in `52-stub-shell.css`); the alias / resolution module (Brief 104 — a **Step 3** seed, not used in Step 1); Cluster-C framing in the 2026-05-29 planning note (`sessions/README.md`).
- **Visual-reference surfaces (the quality bar — see Design freedom).** The project's reworked, higher-polish surfaces are the bar, **not** the old stub/legacy shells: the Chronicle/timeline (`src/app/timeline/page.tsx` + `src/components/timeline/chronicle/*`), the reworked Ask funnel (`src/app/ask/page.tsx` + `src/components/ask/*`), and the `/buecher` list (`src/app/buecher/`). The shared vocabulary underneath them is the `chrono/*` primitives + `00-tokens.css` — lean on those, match the polish, don't invent a parallel look.
- **Step 1 depends on neither Brief 108 nor Brief 104.** 104's alias module is only needed at **Step 3** (search), so it lives in that section. Brief 108 was redirected 2026-06-01: its `globals.css` monolith was already split into `src/app/styles/**` on `main`, the end-state is **ownership-split globals (not CSS Modules)**, and the Playwright baseline is dropped in favour of eyeballing the Vercel preview. This brief follows that decision — ownership-split CSS + browser-smoke, no Modules, no visual-regression suite.
