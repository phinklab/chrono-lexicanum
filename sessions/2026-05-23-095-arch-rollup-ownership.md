...
**Worktree-Selbstprüfung am Session-Start (Vorbedingung der Regel).** Damit die worktree-gebundene Regel verlässlich greift, muss CC seinen Worktree am Session-Start nicht nur *erkennen*, sondern auch *sichtbar machen*. Die bestehende Session-Start-Routine (`git branch --show-current` + `git status`, Strang aus dem Pfad ableiten) wird um zwei Punkte ergänzt: (1) CC **sagt den erkannten Worktree, Strang und die Task-Branch in einem Satz an**, bevor es Dateien anfasst — so erkennt der Maintainer einen Ordner-Fehlgriff sofort. (2) Passt die gestellte Aufgabe nicht zum erkannten Strang (UI-Aufgabe im Batches-Worktree, Batch-/Resolver-Aufgabe im Product-Worktree, und umgekehrt), **hält CC an und fragt zurück**, statt am falschen Ort zu beginnen. CC fragt den Maintainer **nie** „in welchem Worktree bin ich" — der Strang steht im Pfad, CC leitet ihn selbst ab und meldet ihn.
...
- **Worktree-Selbstprüfung härten** — in denselben Dateien (`CLAUDE.md` § „Parallel worktrees", `AGENTS.md` § „Parallel worktree git protocol", `cc-session.md` Session-Start): die bestehende Session-Start-Routine wird ergänzt um (a) das explizite **Ansagen** des erkannten Worktree + Strang + Task-Branch, bevor Dateien angefasst werden, und (b) einen **Halt mit Rückfrage**, wenn die gestellte Aufgabe nicht zum erkannten Strang passt. CC leitet den Strang **immer selbst** aus dem Worktree-Pfad ab — es fragt den Maintainer nie danach. Es ist eine Selbst-Prüfung, keine Maintainer-Rückfrage.
...
- [ ] `CLAUDE.md` / `AGENTS.md` / `cc-session.md` tragen die gehärtete Worktree-Selbstprüfung: erkannten Worktree + Strang + Task-Branch am Session-Start ansagen; Halt-mit-Rückfrage bei Aufgabe-/Strang-Mismatch; CC leitet den Strang selbst ab und fragt den Maintainer nie danach.
