...
Verification Runde 10: `typecheck` + `lint` grün (Exit 0); ein Dev-Server, `/map` → 200. Zusammen mit der Runde-9-Filter-UX uncommitted im Working Tree — Commit auf Philipps Zuruf. *(Nachtrag: Zuruf kam, committed als `6ef17ee` und gepusht.)*

## Nachtrag 10 — Feedback-Runde 11 (vor PR, 2026-07-08)

Philipp: Zonen-Namen-Typos fixen; das Hintergrund-Foto raus, nur eine Flächenfarbe; alle anderen Direction-Proofs-Settings raus.

**1. Zonen-Namen gefixt** (zones.json, Philips Freigabe der drei gemeldeten Typos): „Sauthekh Dynasty" → **Sautekh Dynasty**, „Storm of the Emperors Wrath" → **Storm of the Emperor's Wrath**, „Sirens storm" → **Siren's Storm**.

**2. Direction-Proofs komplett ausgebaut, Karte auf einer Flächenfarbe.** Der Richtungs-Entscheid ist gefallen (das Panel trug seit Brief 178 den Vermerk „fliegt nach dem Richtungs-Entscheid raus"): `/map` rendert **kein SiteBackground mehr** (cartog-holo-Foto raus), `.map-route` steht auf `background: var(--cl-void)` — dieselbe Grundfarbe, auf der das gesamte Site-Design aufbaut, definiert einmal in 00-tokens.css. Gelöscht: `DirectionPanel.tsx` samt `.cg-debug`-CSS (+ Mobile-Eintrag), die `bgArt`/`veil`/`bright`/`grain`-Felder aus State/Actions/Reducer, der Root-CSS-Var-Effect (`--cg-veil`/`--cg-bright`/`--cg-grain` + `html.cg-nobg`), die Overlay-Divs `.cg-veil`/`.cg-grainlayer` samt CSS und die Foto-Dim-Regeln (`.map-route .site-bg__photo`). Die `cartog-holo`-Variante bleibt im geteilten SiteBackground-Registry (andere Routen könnten sie nutzen), ihr Kommentar ist als unused markiert.

Verification Runde 11: `typecheck` + `lint` grün (Exit 0); ein Dev-Server, `/map` → 200.

**Rollup-Fakten (für Cowork):** Map-World-Blurbs sind **komplett** (Runbook `docs/map-world-blurbs-run.md`, headless Sonnet 5 @ medium; `src/lib/map/world-blurbs.json` mit 923 Einträgen im Welt-id-Keyspace, lazy Client-Chunk via `src/lib/map/world-blurbs.ts` — location-blurbs haben Vorrang); Zoomer hat Presets 3.2×/6.0× auf den Band-Schwellen; Legende-Wording: „Filter worlds"/„By world type"/„Show on the chart"; Census hat einen Namens-Zwang-Toggle („World names at every magnification" → `svg.names` hebt die Label-Band-Gates auf) und einen Zonen-Toggle („Zones & storm fields" → `svg.nozones` fadet `#cg-fields`); Graticule weicht der Silhouette (`ringArcs`, `RING_CLEAR=26`, Tempestus-Innenkante = Solar-Ring); **alle Zonen-Grafiken sind Hand-Kuration**: GreatRift.tsx/Storms/Areas/„Rift unrest" gelöscht, `zones.json` trägt Philipps 15 kuratierte Zonen (inkl. neuem Kind `plague` = Nurgle-Galle `#8fae4a`, -45°-Schraffur `#cg-plagueHatch`; beide Patterns leben in decor.tsx → `HatchDefs`); die Rift-Spine-Kurve (`RIFT_D`) ist noch Studien-Geometrie und soll aus zone-2 „Cicatrix Maledictum" abgeleitet werden (⚠-TODOs in chart-geometry.ts/LumenNihilus.tsx); Zonen-Editor ist hart dev-only (NODE_ENV-Gate, `?zones=edit` wirkungslos im Prod-Build), seine Zonen-Flächen rendern unter den Planeten (#cg-fields-Portal); **Direction-Proofs sind Geschichte** (Runde 11): `/map` ohne SiteBackground auf `--cl-void`-Flächenfarbe, DirectionPanel/veil/bright/grain/cg-nobg gelöscht.
