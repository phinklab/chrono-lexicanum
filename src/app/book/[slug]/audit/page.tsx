  ...
  return (
    <main id="main" tabIndex={-1} className="audit-shell">
      <header className="audit-header">
        ...
        <Link href={`/book/${work.slug}`} className="audit-back-link">
          Public view
        </Link>
      </header>
              ...
              <ul className="audit-linked-list">
                {audit.containedIn.map((row) => (
                  <li key={row.collectionSlug}>
                    <Link href={`/book/${row.collectionSlug}`}>{row.collectionTitle}</Link>
              ...
              <ul className="audit-linked-list">
                {audit.contains.map((row) => (
                  <li key={row.contentSlug}>
                    <Link href={`/book/${row.contentSlug}`}>{row.contentTitle}</Link>
                    <span>order {row.displayOrder}</span>
                    <span>conf {formatConfidence(row.confidence)}</span>
                    <p>{formatValue(row.basis)}</p>
                  </li>
