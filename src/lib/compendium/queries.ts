 ...
 * row objects that are shape-checked and field-coerced here. Every query
 * degrades to an empty list on DB failure so an unreachable database at build
 * time renders an empty category instead of failing the build.
 */
import "server-only";
 ...
 * runtime and throws loudly into each query's try/catch → degraded-empty
 * path; field-level coercion stays with `toNumber`/`parsePgTextArray`.
 */
...
export async function getWeltenRows(): Promise<WeltenRow[]> {
  try {
    ...
    console.error(`[compendium/welten] DB fetch failed (${msg}); rendering empty.`);
    return [];
  }
}
...
export async function getCharaktereRows(): Promise<CharaktereRow[]> {
  try {
    ...
    const msg = err instanceof Error ? `${err.name}: ${err.message}` : String(err);
    console.error(`[compendium/charaktere] DB fetch failed (${msg}); rendering empty.`);
    return [];
  }
}
...
export async function getPersonenRows(): Promise<PersonenRow[]> {
  try {
    ...
    const msg = err instanceof Error ? `${err.name}: ${err.message}` : String(err);
    console.error(`[compendium/personen] DB fetch failed (${msg}); rendering empty.`);
    return [];
  }
}
