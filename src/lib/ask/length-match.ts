 ...
 * "standalone" — it forces a single volume. For "trilogy" and "epic" the
 * boundary lets everything through and length becomes a soft scoring signal
 * (arc / series weights), so a high-scoring book of the wrong commitment can
 * still top the list. That is the "doesn't really fit" problem.
 *
 ...
 *   - epic    → the book is part of a series (matched the `series` tag)
 *
...
function isGradedLength(value: AskAnswers["length"]): value is GradedLength {
  return value === "trilogy" || value === "epic";
}

export interface LengthPartition {
  /**
   * True only for "trilogy"/"epic", where the length is a soft signal some
   * results may miss — the only case where we banner or split the list.
   */
  graded: boolean;
